(function (editors, elFinder) {
  if (typeof define === 'function' && define.amd) {
    ...
    elFinder.prototype._options.commandsOptions.edit.editors = optEditors.concat(
      editors(elFinder),
    );
  }
})(function (elFinder) {
  ...
  var apps = {},
    ...
    hasFlash = (function () {
      ...
      try {
        ...
        hasFlash = !!(
          ...
          (navigator && navigator.mimeTypes['application/x-shockwave-flash'])
        );
      }
      return hasFlash;
    })(),
    ...
    initImgTag = function (id, file, content, fm) {
      var node = $(this)
          .children('img:first')
          .data('ext', getExtention(file.mime, fm)),
        spnr = $('<div class="elfinder-edit-spinner elfinder-edit-image"/>')
          .html(
            '<span class="elfinder-spinner-text">' +
              fm.i18n('ntfloadimg') +
              '</span><span class="elfinder-spinner"/>',
          )
          .hide()
          .appendTo(this),
        url;
      ...
      node
        ...
        .data('loading', function (done) {
          var btns = node
            ...
            .find('button,.elfinder-titlebar-button');
          btns
            .prop('disabled', !done)
            [done ? 'removeClass' : 'addClass']('ui-state-disabled');
      ...
      var self = this,
        ...
        launch = function () {
          ...
          container
            ...
            .on('error', error)
            .appendTo(
              elfNode.hasClass('elfinder-fullscreen') ? elfNode : 'body',
            );
        },
        errtm;
  ...
  return [
      ...
      setup: function (opts, fm) {
        if (fm.UA.ltIE8 || fm.UA.Mobile) {
          ...
          if (!fm.isSameOrigin(this.opts.iconsPath)) {
            ...
            fm.debug(
              'warning',
              'Setting `commandOptions.edit.extraOptions.tuiImgEditOpts.iconsPath` MUST follow the same origin policy.',
            );
          }
        }
      },
        ...
        var self = this,
          ...
          init = function (editor) {
            var $base = $(base),
              ...
              tmpDiv = [
                $('<div class="tui-image-editor-submenu"/>').appendTo(
                  tmpContainer,
                ),
                $('<div class="tui-image-editor-controls"/>').appendTo(
                  tmpContainer,
                ),
              ],
              iEditor = new editor(base, {
                ...
                cssMaxHeight: Math.max(
                  200,
                  bParent.height() -
                    (tmpDiv[0].height() + tmpDiv[1].height() + 3) /*margin*/,
                ),
              ...
              zup = $('<span class="ui-icon ui-icon-plusthick"/>').data(
                'val',
                10,
              ),
              zdown = $('<span class="ui-icon ui-icon-minusthick"/>').data(
                'val',
                -10,
              ),
              per = $('<button/>')
            ...
            if (self.file.mime === 'image/jpeg') {
              $base.data(
                'quality',
                fm.storage('jpgQuality') || fm.option('jpgQuality'),
              );
              quty = $(
                '<input type="number" class="ui-corner-all elfinder-resize-quality elfinder-tabstop"/>',
              )
                .attr('min', '1')
                .attr('max', '100')
                .attr('title', '1 - 100')
                    ...
                    canvas.toBlob(
                      function (blob) {
                        blob &&
                          quty
                            .next('span')
                            .text(' (' + fm.formatSize(blob.size) + ')');
                      },
                      'image/jpeg',
                      Math.max(Math.min(q, 100), 1) / 100,
                    );
                  });
                })
            ...
            require.config({
              paths: {
                'fabric/dist/fabric.require':
                  cdns.fabric + '/fabric.require.min', // for fabric < 2.0.1
                fabric: cdns.fabric + '/fabric.min', // for fabric >= 2.0.1
                'tui-code-snippet':
                  cdns.tui + '/tui.code-snippet/latest/tui-code-snippet.min',
                'tui-color-picker':
                  cdns.tui + '/tui-color-picker/latest/tui-color-picker.min',
                'tui-image-editor':
                  cdns.tui +
                  '/tui-image-editor/' +
                  ver +
                  '/tui-image-editor.min',
              },
            });
            ...
            fm.loadScript(
              ...
              function () {
                fm.loadScript(
                  [
                    cdns.tui +
                      '/tui-color-picker/latest/tui-color-picker.min.js',
                  ],
                  function () {
                    fm.loadScript(
                      [
      ...
      getContent: function (base) {
        ...
        if (editor.instance) {
          if ($base.data('mime') === 'image/jpeg') {
            quality =
              quality || fm.storage('jpgQuality') || fm.option('jpgQuality');
    ...
    },
    {
      ...
      init: function (id, file, dum, fm) {
        var orig = 'https://www.photopea.com',
          ...
          spnr = $(
            '<div class="elfinder-edit-spinner elfinder-edit-photopea"/>',
          )
            .html(
              '<span class="elfinder-spinner-text">' +
                fm.i18n('nowLoading') +
                    ...
                    if (typeof ev.data === 'object') {
                      dfdGet.resolve(
                        'data:' +
                          mime +
                          ';base64,' +
                          fm.arrayBufferToBase64(ev.data),
                      );
                    } else {
                      dfdGet.reject('errDataEmpty');
                    }
            ...
            this.getContent = function () {
              ...
              if (phase > 1) {
                ...
                if (phase === 2) {
                  phase = 3;
                  dfdGet.resolve(
                    'data:' + mime + ';base64,' + fm.arrayBufferToBase64(data),
                  );
                  data = null;
                  return dfdGet;
                }
                ...
                wnd.postMessage(
                  'app.activeDocument.saveToOE("' + type + '")',
                  orig,
                );
                return dfdGet;
              }
      ...
      close: function (base, liveMsg) {
        $(base).attr('src', '');
        liveMsg &&
          $(window).off('message.' + this.fm.namespace, liveMsg.receive);
      },
    },
    {
      ...
      setup: function (opts, fm) {
        if (
          fm.UA.ltIE8 ||
          !opts.extraOptions ||
          !opts.extraOptions.pixo ||
          !opts.extraOptions.pixo.apikey
        ) {
          this.disabled = true;
        } else {
      ...
      load: function (base) {
        var self = this,
          ...
          init = function (onload) {
            ...
            if (!container.length) {
              container = $(
                '<div id="elfinder-pixo-container" class="ui-front"/>',
              )
                .css({
                  position: 'fixed',
                  top: 0,
              ...
              elfNode.on('resize.' + fm.namespace, function (e, data) {
                ...
                data &&
                  data.fullscreen &&
                  container.appendTo(
                    data.fullscreen === 'on' ? elfNode : 'body',
                  );
              });
            ...
            opts = Object.assign(
              {
                ...
                onSave: function (arg) {
                  ...
                  delete base._canvas;
                  if (node.data('ext') !== ext) {
                    changeImageType(url, self.file.mime)
                      .done(function (res, cv) {
                        if (cv) {
        ...
        if (typeof Pixo === 'undefined') {
          fm.loadScript(
            ['https://pixoeditor.com:8443/editor/scripts/bridge.m.js'],
            function () {
              init(launch);
            },
            {loadType: 'tag'},
      ...
      save: function (base) {
        ...
        if (base._canvas) {
          ...
          node.attr(
            'src',
            base._canvas.toDataURL(
              self.file.mime,
              q ? Math.max(Math.min(q, 100), 1) / 100 : void 0,
            ),
          );
    ...
    },
    {
      ...
      setup: function (opts, fm) {
        if (
          fm.UA.ltIE8 ||
          !opts.extraOptions ||
          !opts.extraOptions.creativeCloudApiKey
        ) {
          this.disabled = true;
        } else {
      ...
      load: function (base) {
        var self = this,
          ...
          init = function (onload) {
            ...
            if (!container.length) {
              container = $(
                '<div id="elfinder-aviary-container" class="ui-front"/>',
              )
                .css({
                  position: 'fixed',
                  top: 0,
          ...
          start = function () {
            ...
            ace.config.loadModule('ace/ext/modelist', function () {
              ...
              mode = ace
                ...
                .getModeForPath('/' + self.file.name).name;
              if (mode === 'text') {
      ...
      load: function (textarea) {
        var fm = this.fm,
          ...
          start = function (CodeMirror) {
            ...
            if (info) {
              CodeMirror.modeURL = useRequire
                ? 'codemirror/mode/%N/%N.min'
                : cmUrl + '/mode/%N/%N.min.js';
              mode = info.mode;
              spec = info.mime;
    ...
    },
    {
      ...
      load: function (textarea) {
          ...
          start = function (SimpleMDE) {
            ...
            textarea._setHeight = function (height) {
              var h = height || base.height(),
                ...
                areaH;
              base
                .children('.editor-toolbar,.editor-statusbar')
                .each(function () {
        ...
        if (!self.confObj.loader) {
          ...
          if (useRequire) {
            require([cdn + '/simplemde.min.js'], function (SimpleMDE) {
      ...
      setup: function (opts, fm) {
        ...
        if (!fm.options.cdns.ckeditor) {
          ...
          if (opts.extraOptions) {
            confObj.ckeOpts = Object.assign(
              {},
              opts.extraOptions.ckeditor || {},
            );
      ...
      load: function (textarea) {
        var self = this,
          ...
          init = function () {
            ...
            opts = {
              ...
              on: {
                instanceReady: function (e) {
                  ...
                  dlg
                    .one('beforedommove.' + fm.namespace, function () {
                      editor.destroy();
                    })
            ...
            CKEDITOR.replace(
              textarea.id,
              Object.assign(opts, self.confObj.ckeOpts),
            );
            CKEDITOR.on('dialogDefinition', function (e) {
              var dlg = e.data.definition.dialog;
              dlg.on('show', function (e) {
                fm.getUI()
                  .append($('.cke_dialog_background_cover'))
            ...
            cEditor
              ...
              .then(function (editor) {
                ...
                if (
                  ...
                  (mode === 'classic' || mode === 'decoupled-document')
                ) {
                  $(editnode)
                    .closest('.elfinder-dialog')
                ...
                fm.log({
                  ...
                  plugins: cEditor.builtinPlugins.map(function (p) {
                    return p.pluginName;
                  }),
              ...
              ['catch'](function (error) {
                // ['cache'] instead .cache for fix error on ie8
                fm.error(error);
              });
          },
    ...
    },
    {
      ...
      setup: function (opts, fm) {
        ...
        if (!fm.options.cdns.tinymce) {
          ...
          confObj.mceOpts = {};
          if (opts.extraOptions) {
            confObj.uploadOpts = Object.assign(
              {},
              opts.extraOptions.uploadOpts || {},
            );
            confObj.mceOpts = Object.assign(
              {},
              opts.extraOptions.tinymce || {},
            );
          } else {
            confObj.uploadOpts = {};
          }
      ...
      load: function (textarea) {
        var self = this,
          ...
          init = function () {
            ...
            textarea._setHeight = function (height) {
              if (tVer < 5) {
                var base = $(this).parent(),
                  ...
                  areaH;
                base
                  .find('.mce-container-body:first')
                  .children('.mce-top-part,.mce-statusbar')
                  .each(function () {
                    ctrH += $(this).outerHeight(true);
                  });
                areaH = h - ctrH - delta;
            ...
            opts = {
              ...
              toolbar:
                'formatselect | bold italic strikethrough forecolor backcolor | link image media | alignleft aligncenter alignright alignjustify | numlist bullist outdent indent | removeformat',
              image_advtab: true,
        ...
        if (!self.confObj.loader) {
          ...
          self.fm.loadScript(
            [
              fm.options.cdns.tinymce +
                (fm.options.cdns.tinymce.match(/\.js/)
                  ? ''
                  : '/tinymce.min.js'),
            ],
            function () {
              ...
              if (data.zohourl) {
                ...
                if (data.warning) {
                  ...
                  fm.toast({
                    ...
                    timeOut: 0,
                    onHidden: function () {
                      uiToast.children().length === 1 &&
                        uiToast.appendTo(fm.getUI());
                    },
                    button: {
                      text: 'btnYes',
                    },
    ...
    },
    {
      ...
      // setup on elFinder bootup
      setup: function (opts, fm) {
        var mOpts = opts.extraOptions.onlineConvert || {
          maxSize: 100,
          showLink: true,
        };
        if (mOpts.maxSize) {
        ...
        this.set = Object.assign(
          {
            ...
            conv: {
              ...
              Video: {
                ...
                WebM: {},
                WMV: {},
                Android: {link: '/convert-video-for-%s', ext: 'mp4'},
                Blackberry: {link: '/convert-video-for-%s', ext: 'mp4'},
                DPG: {link: '/convert-video-for-%s', ext: 'avi'},
                iPad: {link: '/convert-video-for-%s', ext: 'mp4'},
                iPhone: {link: '/convert-video-for-%s', ext: 'mp4'},
                iPod: {link: '/convert-video-for-%s', ext: 'mp4'},
                'Nintendo-3DS': {link: '/convert-video-for-%s', ext: 'avi'},
                'Nintendo-DS': {link: '/convert-video-for-%s', ext: 'avi'},
                PS3: {link: '/convert-video-for-%s', ext: 'mp4'},
                Wii: {link: '/convert-video-for-%s', ext: 'avi'},
                Xbox: {link: '/convert-video-for-%s', ext: 'wmv'},
              },
            },
            catExts: {
              Hash: 'txt',
            ...
            link:
              '<div class="elfinder-edit-onlineconvert-link"><a href="https://www.online-convert.com" target="_blank"><span class="elfinder-button-icon"></span>ONLINE-CONVERT.COM</a></div>',
      ...
      init: function (id, file, dum, fm) {
        var ta = this,
            ...
            dfdInit.done(function (data) {
              ...
              if (confObj.api) {
                ...
                if (!confObj.conversions[cat]) {
                  dfd = $.getJSON(
                    'https://api2.online-convert.com/conversions' + type,
                  );
                } else {
                ...
                dfd.done(function (d) {
                  ...
                  $.each(d, function (i, o) {
                    btns[set.useTabs ? 'children' : 'find'](
                      '.onlineconvert-category-' + o.category,
                    )
                      .children('.onlineconvert-' + o.target)
                      .trigger('makeoption', o);
                  });
                  done && done();
                });
              }
          ...
          btns = (function () {
            var btns = $('<div/>')
                ...
                .on('change', function (e) {
                  var t = $(e.target),
                    p = t.parent(),
                    b = t
                      .closest('.elfinder-edit-onlineconvert-button')
                      .children('button:first'),
                    o = b.data('opts') || {},
                    v =
                      p.data('type') === 'boolean' ? t.is(':checked') : t.val();
                  e.stopPropagation();
                  if (v) {
              ...
              oform = function (n, o) {
                ...
                if (o.type === 'boolean') {
                  if (
                    o['default'] ||
                    (nozip = n === 'allow_multiple_outputs' && !allowZip)
                  ) {
                    checked = ' checked';
                    if (nozip) {
                      disabled = ' disabled';
              ...
              makeOption = function (o) {
                var elm = this,
                  b = $(
                    '<span class="elfinder-button-icon elfinder-button-icon-preference"/>',
                  ).on('click', function () {
                    f.toggle();
                  }),
                  f = $(
                    '<div class="elfinder-edit-onlinconvert-options"/>',
                  ).hide();
                if (o.options) {
                  $.each(o.options, function (k, v) {
                    k !== 'download_password' &&
                      f.append(oform.call(elm, k, v));
                  });
                }
                elm.append(b, f);
              },
            ...
            if (!confObj.ext2mime) {
              confObj.ext2mime = Object.assign(
                fm.arrayFlip(fm.mimeTypes),
                ext2mime,
              );
            }
            $.each(set.conv, function (t, c) {
              var cname = t.toLowerCase(),
                ...
                type = $(
                  '<div id="' +
                    id +
                    '" class="onlineconvert-category onlineconvert-category-' +
                    cname +
                    '"/>',
                ).data('cname', t),
                cext;
              $.each(c, function (n, o) {
                ...
                if (fm.uploadMimeCheck(confObj.ext2mime[ext], file.phash)) {
                  type.append(
                    $(
                      '<div class="elfinder-edit-onlineconvert-button onlineconvert-' +
                        nl +
                        '"/>',
                    )
                      .on('makeoption', function (e, data) {
                        var elm = $(this);
                        if (
                          !elm.children('.elfinder-button-icon-preference')
                            .length
                        ) {
            ...
            if (set.useTabs) {
              ...
              $.each(set.conv, function (t) {
                ...
                btns.append(
                  $('<fieldset class="onlineconvert-fieldset-' + tl + '"/>')
                    .append($('<legend/>').text(t))
                    .append(btns.children('.onlineconvert-category-' + tl)),
                );
              });
            }
            return btns;
          ...
          spnr = $(
            '<div class="elfinder-edit-spinner elfinder-edit-onlineconvert"/>',
          )
            .hide()
            .html(
              '<span class="elfinder-spinner-text">' +
              ...
              url().done(function (url) {
                ...
                fm.request({
                  data: {
                    ...
                    'args[filename]':
                      fm.splitFileExtention(file.name)[0] +
                      '.' +
                      getExt(opts.category, opts.convert),
                    'args[mime]': file.mime,
                  },
                  preventDefault: true,
                })
        ...
        if (set.useTabs) {
          ...
          ifm
            .parent()
            .scrollTop(
              btns.children('.onlineconvert-fieldset-' + mode).offset().top,
            );
        }
      },
      load: function () {},
