...
elFinder.prototype.resources = {
  ...
  tpl: {
    ...
    navdir:
      '<div class="elfinder-navbar-wrapper{root}"><span id="{id}" class="ui-corner-all elfinder-navbar-dir {cssclass}"{title}><span class="elfinder-navbar-arrow"/><span class="elfinder-navbar-icon" {style}/>{symlink}{permissions}{name}</span><div class="elfinder-navbar-subtree" style="display:none"/></div>',
    placedir:
      '<div class="elfinder-navbar-wrapper"><span id="{id}" class="ui-corner-all elfinder-navbar-dir {cssclass}"{title}><span class="elfinder-navbar-arrow"/><span class="elfinder-navbar-icon" {style}/>{symlink}{permissions}{name}</span><div class="elfinder-navbar-subtree" style="display:none"/></div>',
  },
  ...
  mixin: {
    make: function () {
      ...
      var self = this,
        ...
        phash =
          this.data && this.data.target ? this.data.target : tree ? fm.file(sel[0]).hash : fm.cwd().hash,
        date = new Date(),
        file = {
          hash: id,
          phash: phash,
                  ...
                  fm.request({
                    ...
                    .done(function (data) {
                      if (data && data.added && data.added[0]) {
                        ...
                        if (!move) {
                          if (
                            fm.mimeIsText(item.mime) &&
                            !fm.mimesCanMakeEmpty[item.mime] &&
                            fm.mimeTypes[item.mime]
                          ) {
                            ...
                            fm.storage(
                              "mkfileTextMimes",
                              Object.assign(tmpMimes, fm.storage("mkfileTextMimes") || {})
                            );
                          }
                          Object.assign(
                            nextAct,
                            nextAction ||
                              acts[item.mime] ||
                              acts[item.mime.split("/")[0]] ||
                              acts[
                                fm.mimesCanMakeEmpty[item.mime] ||
                                $.inArray(item.mime, fm.resources.mimes.text) !== -1
                                  ? "text"
                                  : "none"
                              ] ||
                              acts["default"]
        ...
        select = function () {
          ...
          if (!inError && fm.UA.Mobile && !fm.UA.iOS) {
            // since iOS has a bug? (z-index not effect) so disable it
