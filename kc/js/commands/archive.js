...
elFinder.prototype.commands.archive = function () {
  ...
  this.getstate = function (select) {
    ...
    if (chk && fm.searchStatus.state > 1) {
      ...
      chk =
        cnt ===
        $.grep(sel, function (f) {
          return f.read && f.hash.indexOf(cwdId) === 0 ? true : false;
        }).length;
    }
  ...
  this.exec = function (hashes, type) {
    ...
    self.mime = mime;
    self.prefix =
      (cnt > 1 ? 'Archive' : files[0].name) +
      (fm.option('archivers')['createext']
        ? '.' + fm.option('archivers')['createext'][mime]
        : '');
