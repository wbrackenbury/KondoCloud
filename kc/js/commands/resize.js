...
elFinder.prototype.commands.resize = function () {
  ...
  this.resizeRequest = function (data, f, dfrd) {
    ...
    if (
      enabled &&
      (!file ||
        (file && file.read && file.write && file.mime.indexOf('image/') !== -1))
    ) {
      return fm
        .request({
        ...
        .done(function () {
          if (data.quality) {
            fm.storage(
              'jpgQuality',
              data.quality === fm.option('jpgQuality') ? null : data.quality,
            );
          }
          dfrd && dfrd.resolve();
        });
  ...
  this.exec = function (hashes) {
    var self = this,
      ...
      grid8Def =
        typeof options.grid8px === 'undefined' || options.grid8px !== 'disable'
          ? true
          : false,
      ...
      open = function (file, id) {
        var isJpeg = file.mime === 'image/jpeg',
            ...
            .on('change', function () {
              ...
              changeTm = requestAnimationFrame(function () {
                ...
                if (sizeImg && !operate && (canvas = sizeImg.data('canvas'))) {
                  panel = control.children(
                    'div.elfinder-resize-control-panel:visible',
                  );
                  ...
                  if (quty.is(':visible')) {
                    ...
                    canvas.toBlob(
                      ...
                      'image/jpeg',
                      Math.max(Math.min(quty.val(), 100), 1) / 100,
                    );
                  }
                }
              });
          ...
          preview = $('<div class="elfinder-resize-preview"/>').on(
            'touchmove',
            function (e) {
          ...
          spinner = $(
            '<div class="elfinder-resize-loading">' +
              fm.i18n('ntfloadimg') +
              '</div>',
          ),
          ...
          uiresize = $(
            '<div class="elfinder-resize-uiresize elfinder-resize-control-panel"/>',
          ),
          uicrop = $(
            '<div class="elfinder-resize-uicrop elfinder-resize-control-panel"/>',
          ),
          uirotate = $(
            '<div class="elfinder-resize-rotate elfinder-resize-control-panel"/>',
          ),
          uideg270 = $('<button/>')
            .attr('title', fm.i18n('rotate-cw'))
            .append(
              $(
          ...
          uiprop = $('<span />'),
          reset = $('<button class="elfinder-resize-reset">')
            .text(fm.i18n('reset'))
            .on('click', function () {
              resetView();
            })
          ...
          uitype = $('<div class="elfinder-resize-type"/>').append(
            '<input type="radio" name="type" id="' +
              id +
              '-resize" value="resize" checked="checked" /><label for="' +
              id +
              '-resize">' +
              fm.i18n('resize') +
              '</label>',
            '<input class="api2" type="radio" name="type" id="' +
              id +
              '-crop" value="crop" /><label class="api2" for="' +
              id +
              '-crop">' +
              fm.i18n('crop') +
              '</label>',
            '<input class="api2" type="radio" name="type" id="' +
              id +
              '-rotate" value="rotate" /><label class="api2" for="' +
              id +
              '-rotate">' +
              fm.i18n('rotate') +
              '</label>',
          ...
          type = uitype[ctrgrup]()
            ...
            .on('change', function () {
              ...
              if (mode == 'resize') {
                ...
                resizable();
                isJpeg &&
                  grid8px.insertAfter(uiresize.find('.elfinder-resize-grid8'));
              } else if (mode == 'crop') {
                uirotate.hide();
                uiresize.hide();
                uicrop.show();
                croppable();
                isJpeg &&
                  grid8px.insertAfter(uicrop.find('.elfinder-resize-grid8'));
              } else if (mode == 'rotate') {
                uiresize.hide();
                uicrop.hide();
                uirotate.show();
          ...
          pointX = $(input).on('change', function () {
            crop.updateView();
          }),
          pointY = $(input).on('change', function () {
            crop.updateView();
          }),
          offsetX = $(input).on('change', function () {
            crop.updateView('w');
          }),
          offsetY = $(input).on('change', function () {
            crop.updateView('h');
          }),
          quality =
            isJpeg && api2
              ? $(input)
                  .val(
                    fm.storage('jpgQuality') > 0
                      ? fm.storage('jpgQuality')
                      : fm.option('jpgQuality'),
                  )
              ...
              : null,
          degree = $(
            '<input type="number" class="ui-corner-all" maxlength="3" value="0" />',
          ).on('change', function () {
            rotate.update();
          }),
          uidegslider = $(
            '<div class="elfinder-resize-rotate-slider touch-punch"/>',
          )
            .slider({
              min: 0,
              max: 360,
            ...
            .on('keydown', function (e) {
              if (
                ...
                rotate.update(
                  Number(degree.val()) +
                    (e.keyCode == $.ui.keyCode.RIGHT ? 1 : -1),
                  false,
                );
              }
            })
          ...
          getColorCode = function (r, g, b) {
            return $.map([r, g, b], function (c) {
              return ('0' + parseInt(c).toString(16)).slice(-2);
            }).join('');
          },
          picker = $('<button>')
            .text(fm.i18n('colorPicker'))
            .on('click', function () {
              imgr
                .on('mousemove.picker click.picker', pick)
                .addClass('elfinder-resize-picking');
              pallet
                .on('mousemove.picker click.picker', 'span', palpick)
                .addClass('elfinder-resize-picking');
            })
            .button({
              icons: {
          ...
          pallet = $('<div class="elfinder-resize-pallet">').on(
            'click',
            'span',
            function () {
          ...
          constr = $('<button>')
            ...
            .on('click', function () {
              ...
              resize.fixHeight();
              rhandle
                .resizable('option', 'aspectRatio', cratio)
                .data('uiResizable')._aspectRatio = cratio;
            })
            .button({
              icons: {
                primary: cratio ? 'ui-icon-locked' : 'ui-icon-unlocked',
          ...
          constrc = $('<button>')
            ...
            .on('click', function () {
              ...
              rhandlec
                .resizable('option', 'aspectRatio', cratioc)
                .data('uiResizable')._aspectRatio = cratioc;
            })
            .button({
              icons: {
                primary: cratioc ? 'ui-icon-locked' : 'ui-icon-unlocked',
          ...
          grid8px = $('<button>')
            .html(fm.i18n(grid8 ? 'enabled' : 'disabled'))
            .toggleClass('ui-state-active', grid8)
            .on('click', function () {
              grid8 = !grid8;
              grid8px
                .html(fm.i18n(grid8 ? 'enabled' : 'disabled'))
          ...
          setStep8 = function () {
            var step = grid8 ? 8 : 1;
            $.each(
              [width, height, offsetX, offsetY, pointX, pointY],
              function () {
                this.attr('step', step);
              },
            );
          ...
          setuprimg = function () {
            ...
            r_scale =
              Math.min(pwidth, pheight) /
              Math.sqrt(Math.pow(owidth, 2) + Math.pow(oheight, 2));
          ...
          setupimg = function () {
            ...
            setuprimg();
            basec.width(img.width()).height(img.height());
            imgc.width(img.width()).height(img.height());
            crop.updateView();
            jpgCalc();
          },
                ...
                if (!pallet.children(':first').length) {
                  ...
                  $.each(
                    ...
                    function () {
                      ...
                      pallet.append(
                        $(
                          '<span style="width:20px;height:20px;display:inline-block;background-color:rgb(' +
                            this.c +
                            ');">',
                        ),
                      );
          ...
          init = function () {
            ...
            oheight =
              file.height || elm.naturalHeight || elm.height || img.height();
            ...
            pheight =
              preview.height() - (rhandle.outerHeight() - rhandle.height());
          ...
          crop = {
            update: function (change) {
              pointX.val(
                round(
                  (rhandlec.data('x') || rhandlec.position().left) / prop,
                  owidth,
                ),
              );
              pointY.val(
                round(
                  (rhandlec.data('y') || rhandlec.position().top) / prop,
                  oheight,
                ),
              );
              if (change !== 'xy') {
                offsetX.val(
                  round(
                    (rhandlec.data('w') || rhandlec.width()) / prop,
                    owidth - pointX.val(),
                  ),
                );
                offsetY.val(
                  round(
                    (rhandlec.data('h') || rhandlec.height()) / prop,
                    oheight - pointY.val(),
                  ),
                );
              }
          ...
          rotate = {
            ...
            setQuality: function () {
              uirotate
                .children('div.elfinder-resize-quality')
                [
                  losslessRotate > 0 && degree.val() % 90 === 0
                    ? 'hide'
                    : 'show'
                ]();
            },
            ...
            execute: function (e) {
              ...
              var rotateAngle =
                mouseAngle - rotate.mouseStartAngle + rotate.imageStartAngle;
              rotateAngle = Math.round(
                (parseFloat(rotateAngle) * 180) / Math.PI,
              );
            ...
            start: function (e) {
              ...
              rotate.mouseStartAngle = Math.atan2(
                mouseStartYFromCentre,
                mouseStartXFromCentre,
              );

              rotate.imageStartAngle =
                (parseFloat(imgr.rotate()) * Math.PI) / 180.0;
          ...
          saveAs = function () {
                ...
                $.proxy(fm.res('mixin', 'make'), self)()
                  .done(function (data) {
                    ...
                    if (data.added && data.added.length) {
                      ...
                      dfd =
                        fm.api < 2.1032
                          ? fm.url(file.hash, {async: true, temporary: true})
                          : null;
                      $.when(dfd)
                        .done(function (url) {
                          fm.request({
                            ...
                            .done(function (data) {
                              ...
                              base
                                .show()
                                .find('.elfinder-dialog-title')
                                .html(fm.escape(file.name));
                              save();
                              dialogs.fadeIn();
                            });
                        })
            ...
            if (checkVals()) {
              dialogs = fmnode
                .children('.' + self.dialogClass + ':visible')
                .removeClass(clsediting)
                .fadeOut();
          ...
          sizeImg = quality
            ? $('<img>')
                .attr('crossorigin', fm.isCORS ? 'use-credentials' : '')
                .attr('src', canvSrc)
                .on('load', function () {
                  try {
          ...
          jpgCalc = function () {
            control
              .find('input.elfinder-resize-quality:visible')
              .trigger('change');
          },
          dinit = function (e) {
            if (
              base.hasClass('elfinder-dialog-minimized') ||
              base.is(':hidden')
            ) {
              return;
            }
            ...
            if (owidth && oheight) {
              pwidth =
                preview.width() - (rhandle.outerWidth() - rhandle.width());
              pheight =
                preview.height() - (rhandle.outerHeight() - rhandle.height());
              resize.updateView(owidth, oheight);
            }
            ...
            if (
              !presetc.children('span.elfinder-resize-preset:visible').length
            ) {
              presetc.hide();
            }
          },
          preset = (function () {
            var sets = $('<fieldset class="elfinder-resize-preset-container">')
                .append($('<legend>').html(fm.i18n('presets')))
                .hide(),
              hasC;
            $.each(presetSize, function (i, s) {
              if (s.length === 2) {
                hasC = true;
                sets.append(
                  $('<span class="elfinder-resize-preset"/>')
                    .data('s', s)
                    .text(s[0] + 'x' + s[1])
                    .button(),
                );
        ...
        uiresize.append(
          ...
          $(row).append(
            $(label).text(fm.i18n('height')),
            height,
            $('<div class="elfinder-resize-whctrls">').append(constr, reset),
          ),
          quality
            ? $(row).append(
          ...
          isJpeg
            ? $(row).append(
                $(label)
                  .text(fm.i18n('8pxgrid'))
                  .addClass('elfinder-resize-grid8'),
                grid8px,
              )
            : $(),
        ...
        if (api2) {
          uicrop.append(
            ...
            $(row).append(
              $(label).text(fm.i18n('height')),
              offsetY,
              $('<div class="elfinder-resize-whctrls">').append(
                constrc,
                reset.clone(true),
              ),
            ),
          ...
          uirotate.append(
            $(row)
              .addClass('elfinder-resize-degree')
              .append(
        ...
        if (api2) {
          ...
          preview.append(
            basec
              .css('position', 'absolute')
              .hide()
              .append(imgc, rhandlec.append(coverc)),
          );
        ...
        if (useSaveAs) {
          buttons[fm.i18n('btnSaveAs')] = function () {
            requestAnimationFrame(saveAs);
          };
        }
        ...
        base = self
          .fmDialog(dialog, {
            ...
            open: function () {
              var substituteImg =
                  fm.option('substituteImg', file.hash) &&
                  file.size > options.dimSubImgSize
                    ? true
                    : false,
                hasSize = file.width && file.height ? true : false;
              dialog.parent().css('overflow', 'hidden');
              dMinBtn = base
                .find('.ui-dialog-titlebar .elfinder-titlebar-minimize')
                .hide();
              ...
              if (api2) {
                imgr
                  .on('mousedown touchstart', rotate.start)
            ...
            close: function () {
              if (api2) {
                imgr
                  .off('mousedown touchstart', rotate.start)
        ...
        control
          .find('input.elfinder-resize-quality')
          .next('span')
          .addClass('elfinder-resize-jpgsize')
          .attr('title', fm.i18n('roughFileSize'));
      },
      id,
      dialog;
...
(function ($) {
  var findProperty = function (styleObject, styleArgs) {
    var i = 0;
    for (i in styleArgs) {
      if (typeof styleObject[styleArgs[i]] != 'undefined') return styleArgs[i];
    }
  ...
  $.fn.rotate = function (val) {
    ...
    if (typeof val == 'undefined') {
      if (!!window.opera) {
        ...
        return r && r[1] ? Math.round((parseFloat(r[1]) * 180) / Math.PI) : 0;
      } else {
        r = this.css('transform').match(/rotate\((.*?)\)/);
  ...
  if (
    ...
    var GetAbsoluteXY = function (element) {
      ...
      while (pnode.offsetParent) {
        ...
        if (
          pnode != document.body &&
          pnode.currentStyle['position'] != 'static'
        ) {
          break;
        }
