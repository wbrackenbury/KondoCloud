  ...
  this.getstate = function (select) {
    ...
    return cnt == 1
      ? sel[0].read
        ...
        : -1
      : cnt && !fm.UA.Mobile
      ? $.grep(sel, function (file) {
          return file.mime == 'directory' || !file.read ? false : true;
        }).length == cnt
        ? 0
        : -1
      : -1;
  ...
  this.exec = function (hashes, cOpts) {
    ...
    if (thash || (cnt == 1 && (file = files[0]) && file.mime == 'directory')) {
      if (!thash && file && !file.read) {
        ...
        if (
          fm.keyState.ctrlKey &&
          (fm.keyState.shiftKey ||
            typeof fm.options.getFileCallback !== 'function')
        ) {
    ...
    files = $.grep(files, function (file) {
      return file.mime != 'directory' ? true : false;
    });
        ...
        if (fm.UA.Mobile || !inline) {
          ...
          if (into === 'window') {
            ...
            wnd = window.open(
              url,
              target,
              w + ',top=50,left=50,scrollbars=yes,resizable=yes,titlebar=no',
            );
          } else {
            if (into === 'tabs') {
              target = file.hash;
    ...
    if (cnt > 1) {
      fm.confirm({
        ...
        buttons:
          fm.getCommand('zipdl') && fm.isCommandEnabled('zipdl', fm.cwd().hash)
            ? [
                {
                  label: 'cmddownload',
                  callback: function () {
