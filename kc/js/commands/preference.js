...
elFinder.prototype.commands.preference = function () {
  var self = this,
    ...
    tab =
      '<li class="' +
      fm.res('class', 'tabstab') +
      ' elfinder-preference-tab-{id}"><a href="#' +
      fm.namespace +
      '-preference-{id}" id="' +
      fm.namespace +
      '-preference-tab-{id}" class="ui-tabs-anchor {class}">{title}</a></li>',
    base = $(
      '<div class="ui-tabs ui-widget ui-widget-content ui-corner-all elfinder-preference">',
    ),
    ul = $(
      '<ul class="ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-top">',
    ),
    tabs = $(
      '<div class="elfinder-preference-tabs ui-tabs-panel ui-widget-content ui-corner-bottom"/>',
    ),
    ...
    selectTab = function (tab) {
      $('#' + fm.namespace + '-preference-tab-' + tab)
        .trigger('mouseover')
        .trigger('click');
      openTab = tab;
    },
    ...
    build = function () {
      ...
      forms.theme &&
        (forms.theme = (function () {
          var cnt = fm.options.themes
            ? Object.keys(fm.options.themes).length
            : 0;
          ...
          var themeSel = $('<select/>').on('change', function () {
            ...
            tpl = {
              image:
                '<img class="elfinder-preference-theme elfinder-preference-theme-image" src="$2" />',
              link: '<a href="$1" target="_blank" title="$3">$2</a>',
              data:
                '<dt>$1</dt><dd><span class="elfinder-preference-theme elfinder-preference-theme-$0">$2</span></dd>',
            },
            items = ['image', 'description', 'author', 'email', 'license'],
            render = function (key, data) {},
            defBtn = $(
              '<button class="ui-button ui-corner-all ui-widget elfinder-preference-theme-default"/>',
            )
              .text(fm.i18n('default'))
              .on('click', function (e) {
                themeSel.val('default').trigger('change');
              }),
            list = $('<div class="elfinder-reference-hide-taball"/>').on(
              'click',
              'button',
              function () {
          ...
          if (!fm.options.themes.default) {
            themeSel.append(
              '<option value="default">' + fm.i18n('default') + '</option>',
            );
          }
          $.each(fm.options.themes, function (id, val) {
            var opt = $(
                '<option class="elfinder-theme-option-' +
                  id +
                  '" value="' +
                  id +
                  '">' +
              ...
              dsc = $(
                '<fieldset class="ui-widget ui-widget-content ui-corner-all elfinder-theme-list-' +
                  id +
                  '"><legend>' +
                  fm.i18n(id) +
                  '</legend><div><span class="elfinder-spinner"/></div></fieldset>',
              ),
              tm;
            themeSel.append(opt);
            list.append(dsc);
            tm = setTimeout(function () {
              dsc
                .find('span.elfinder-spinner')
                .replaceWith(fm.i18n(['errRead', id]));
            }, 10000);
            fm.getTheme(id)
              .always(function () {
                tm && clearTimeout(tm);
              ...
              .done(function (data) {
                ...
                link = data.link
                  ? tpl.link
                      .replace(/\$1/g, data.link)
                      .replace(/\$3/g, fm.i18n('website'))
                  : '$2';
                if (data.name) {
                ...
                dsc
                  .children('legend')
                  .html(link.replace(/\$2/g, fm.i18n(data.name) || id));
                $.each(items, function (i, key) {
                  var t = tpl[key] || tpl.data,
                    elm;
                  if (data[key]) {
                    elm = t
                      .replace(/\$0/g, fm.escape(key))
                      .replace(/\$1/g, fm.i18n(key))
                      .replace(/\$2/g, fm.i18n(data[key]));
                ...
                val = val.add(dl);
                val = val.add(
                  $('<div class="elfinder-preference-theme-btn"/>').append(
          ...
          return $('<div/>').append(
            themeSel.val(fm.theme && fm.theme.id ? fm.theme.id : 'default'),
            defBtn,
            list,
          );
        })());
      ...
      forms.toolbarPref &&
        (forms.toolbarPref = (function () {
          ...
          $.each(pnls, function () {
            ...
            tags.push(
              '<span class="elfinder-preference-toolbar-item"><label><input type="checkbox" value="' +
                cmd +
                '" ' +
                (hides[cmd] ? '' : 'checked') +
                '/>' +
      ...
      forms.columnPref &&
        (forms.columnPref = (function () {
          ...
          $.each(cols, function () {
            ...
            tags.push(
              '<span class="elfinder-preference-column-item"><label><input type="checkbox" value="' +
                key +
                '" ' +
                (hides[key] ? '' : 'checked') +
                '/>' +
      ...
      forms.selectAction &&
        (forms.selectAction = (function () {
          ...
          $.each(acts, function (i, act) {
            ...
            optTags.push(
              '<option value="' + act + '">' + names.join('/') + '</option>',
            );
          });
          return actSel
            .append(optTags.join(''))
            .val(fm.storage('selectAction') || defAct);
        })());

      forms.makefileTypes &&
        (forms.makefileTypes = (function () {
          var hides = fm.getCommand('edit').getMkfileHides(),
            getTag = function () {
              ...
              $.each(fm.mimesCanMakeEmpty, function (mime, type) {
                ...
                tags.push(
                  '<span class="elfinder-preference-column-item" title="' +
                    fm.escape(name) +
      ...
      forms.useStoredEditor &&
        (forms.useStoredEditor = $('<input type="checkbox"/>')
          .prop(
            'checked',
            (function () {
              ...
              return s
                ? s > 0
                : fm.options.commandsOptions.edit.useStoredEditor;
            })(),
          )
      ...
      forms.editorMaximized &&
        (forms.editorMaximized = $('<input type="checkbox"/>')
          .prop(
            'checked',
            (function () {
              ...
              return s
                ? s > 0
                : fm.options.commandsOptions.edit.editorMaximized;
            })(),
          )
          .on('change', function (e) {
      ...
      forms.useFullscreen &&
        (forms.useFullscreen = $('<input type="checkbox"/>')
          .prop(
            'checked',
            (function () {
              ...
              return s
                ? s > 0
                : fm.options.commandsOptions.fullscreen.mode === 'screen';
            })(),
          )
          .on('change', function (e) {
      ...
      if (forms.showHidden) {
        (function () {
          var setTitle = function () {
              ...
              elms
                .prop('disabled', !t.length)
                [t.length ? 'removeClass' : 'addClass']('ui-state-disabled');
              v = t.length ? t.join('\n') : '';
              forms.showHidden.attr('title', v);
              useTooltip &&
                forms.showHidden
                  .tooltip('option', 'content', v.replace(/\n/g, '<br>'))
                  .tooltip('close');
            },
      ...
      forms.infoItems &&
        (forms.infoItems = (function () {
          var items = fm.getCommand('info').items,
            tags = [],
            hides =
              fm.storage('infohides') ||
              fm.arrayFlip(fm.options.commandsOptions.info.hideItems, true);
          $.each(items, function () {
            var key = this,
              name = fm.i18n(key);
            tags.push(
              '<span class="elfinder-preference-info-item"><label><input type="checkbox" value="' +
                key +
                '" ' +
                (hides[key] ? '' : 'checked') +
                '/>' +
      ...
      forms.hashChecker &&
        fm.hashCheckers.length &&
        (forms.hashChecker = (function () {
          var tags = [],
            enabled = fm.arrayFlip(
              fm.storage('hashchekcer') ||
                fm.options.commandsOptions.info.showHashAlgorisms,
              true,
            );
          $.each(fm.hashCheckers, function () {
            var cmd = this,
              name = fm.i18n(cmd);
            tags.push(
              '<span class="elfinder-preference-hashchecker-item"><label><input type="checkbox" value="' +
                cmd +
                '" ' +
                (enabled[cmd] ? 'checked' : '') +
                '/>' +
          ...
          return $(tags.join(' ')).on('change', 'input', function () {
            ...
            fm.storage(
              'hashchekcer',
              $.grep(fm.hashCheckers, function (v) {
                return enabled[v];
              }),
            );
          });
      ...
      forms.autoFocusDialog &&
        (forms.autoFocusDialog = $('<input type="checkbox"/>')
          .prop(
            'checked',
            (function () {
      ...
      $.each(cats, function (id, prefs) {
          ...
          $.each(prefs, function (i, n) {
            ...
            if ((f = forms[n])) {
              ...
              if (cbox.length === 1) {
                ...
                title =
                  '<label for="' + cbox.attr('id') + '">' + title + '</label>';
              ...
              dls = dls
                ...
                .add(
                  $(
                    '<dd class="elfinder-preference-' + n + chks + '"/>',
                  ).append(f),
                );
            }
          });
        ...
        if (found) {
          ul.append(
            tab[r](/\{id\}/g, id)
              [r](/\{title\}/, fm.i18n(id))
              [r](/\{class\}/, openTab === id ? 'elfinder-focus' : ''),
          );
          if (found === 2) {
            tabs.append(
              $(
                '<div id="' +
                  ...
                  '-preference-' +
                  id +
                  '" class="elfinder-preference-content"/>',
              )
                .hide()
      ...
      ul.on('click', 'a', function (e) {
        ...
        .on('focus blur', 'a', function (e) {
          $(this)
            .parent()
            .toggleClass('ui-state-focus', e.type === 'focusin');
        })
