  ...
  this.rec_exec = function (rec_id) {
    ...
    if (cmds.length) {
      var findfunc = function (el) {
          return (
            (el.redo && el.redo.rec_id && el.redo.rec_id == rec_id) ||
            (el.undo && el.undo.rec_id && el.undo.rec_id == rec_id)
          );
        },
        rem_ind = cmds.findIndex(findfunc);

      //console.log(rem_ind);
      //console.log(cmds);

      undo = cmds.splice(rem_ind, 1)[0];
