...
(elFinder.prototype.commands.restore = function () {
  ...
  var self = this,
    ...
    restore = function (dfrd, files, targets, ops) {
      ...
      fakeCnt = 0;
      getFile = getFilesRecursively(files)
        .always(function () {
          tm && clearTimeout(tm);
                ...
                if (!pfile) {
                  ...
                  $.each(fm.trashes, function (ph) {
                    var file = fm.file(ph),
                      filePath = fm.path(ph);
                    if (
                      (!file.volumeid || f.hash.indexOf(file.volumeid) === 0) &&
                      fm.path(f.hash).indexOf(filePath) === 0
                    ) {
                      phash = ph;
                      return false;
                    }
                            ...
                            if (hashes[dir]) {
                              if (files.length) {
                                if (fm.file(hashes[dir])) {
                                  ...
                                  fm.exec('paste', [hashes[dir]], {
                                    _cmd: 'restore',
                                    noToast: opts.noToast || dir !== dirTop,
                                  })
                                    .done(function (data) {
                                      if (
                                        data &&
