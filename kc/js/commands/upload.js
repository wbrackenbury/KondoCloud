  ...
  this.exec = function (data) {
    var fm = this.fm,
      ...
      getTargets = function () {
        ...
        if (!data || data instanceof Array) {
          if (
            !tgts &&
            (sel = fm.selected()).length === 1 &&
            fm.file(sel[0]).mime === 'directory'
          ) {
            tgts = sel;
          } else if (
            !tgts ||
            tgts.length !== 1 ||
            fm.file(tgts[0]).mime !== 'directory'
          ) {
            tgts = [cwdHash];
          }
        }
        ...
        fm.upload(data)
          ...
          .done(function (data) {
            ...
            if (
              ...
              data.added &&
              data.added[0] &&
              !fm.ui.notify.children('.elfinder-notify-upload').length
            ) {
              ...
              if (newItem.length) {
                        ...
                        fm.exec('open', check).done(function () {
                          fm.one('opendone', function () {
                            fm.trigger('selectfiles', {
                              files: $.map(data.added, function (f) {
                                return f.hash;
                              }),
                            });
                          });
                ...
                fm.toast({
                  msg: fm.i18n(['complete', fm.i18n('cmdupload')]),
                  extNode: node,
                });
              }
            }
      ...
      getSelector = function () {
        ...
        return $(
          '<div class="elfinder-upload-dirselect elfinder-tabstop" title="' +
            fm.i18n('folders') +
            '"/>',
        )
          .on('click', function (e) {
            ...
            var $this = $(this),
              ...
              getRaw = function (f, icon) {
                return {
                  ...
                  callback: function () {
                    var title = base
                      .children('.ui-dialog-titlebar:first')
                      .find('span.elfinder-upload-target');
                    targets = [f.hash];
                  ...
                  options: {
                    className:
                      targets && targets.length && f.hash === targets[0]
                        ? 'ui-state-active'
                        : '',
                    iconClass: f.csscls || '',
                    iconImg: f.icon || '',
          ...
          .append(
            '<span class="elfinder-button-icon elfinder-button-icon-dir" />',
          );
      },
    ...
    dropUpload = function (e) {
      ...
      var file = false,
        ...
        kind =
          trf.items && trf.items.length && trf.items[0].kind
            ? trf.items[0].kind
            : '',
        errors;
      ...
      if (trf) {
        ...
        if (
          kind === 'file' &&
          (trf.items[0].getAsEntry || trf.items[0].webkitGetAsEntry)
        ) {
          file = trf;
          type = 'data';
        } else if (
    ...
    dialog = $('<div class="elfinder-upload-dialog-wrapper"/>').append(
      inputButton('multiple', 'selectForUpload'),
    );
    ...
    if (
      ...
      (function (input) {
        return (
          typeof input.webkitdirectory !== 'undefined' ||
          typeof input.directory !== 'undefined'
        );
      })(document.createElement('input'))
    ) {
      dialog.append(
        inputButton('multiple webkitdirectory directory', 'selectFolder'),
      );
    }

    if (targetDir.dirs) {
      if (
        targetDir.hash === cwdHash ||
        fm.navHash2Elm(targetDir.hash).hasClass('elfinder-subtree-loaded')
      ) {
        getSelector().appendTo(dialog);
      } else {
        spinner = $(
          '<div class="elfinder-upload-dirselect" title="' +
            fm.i18n('nowLoading') +
            '"/>',
        )
          .append(
            '<span class="elfinder-button-icon elfinder-button-icon-spinner" />',
          )
          .appendTo(dialog);
    ...
    if (fm.dragUpload) {
      dropbox = $(
        '<div class="ui-corner-all elfinder-upload-dropbox elfinder-tabstop" contenteditable="true" data-ph="' +
        ...
        .prependTo(dialog)
        .after(
          '<div class="elfinder-upload-dialog-or">' + fm.i18n('or') + '</div>',
        )[0];
    } else {
      pastebox = $(
        '<div class="ui-corner-all elfinder-upload-dropbox" contenteditable="true">' +
    ...
    uidialog = this.fmDialog(dialog, {
      title:
        this.title +
        '<span class="elfinder-upload-target">' +
