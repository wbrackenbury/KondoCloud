...
elFinder.prototype.commands.rm = function () {
  ...
  var self = this,
    fm = this.fm,
    tpl =
      '<div class="ui-helper-clearfix elfinder-rm-title"><span class="elfinder-cwd-icon {class} ui-corner-all"/>{title}<div class="elfinder-rm-desc">{desc}</div></div>',
    confirm = function (dfrd, targets, files, tHash, addTexts) {
      ...
      if (cnt > 1) {
        ...
        descs.push(
          fm.i18n('size') + ': ' + (getSize ? spinner : fm.formatSize(size)),
        );
        text = [
          $(
            tpl
              .replace('{class}', 'elfinder-cwd-icon-group')
              .replace(
                '{title}',
                '<strong>' + fm.i18n('items') + ': ' + cnt + '</strong>',
              )
        ...
        descs.push(
          fm.i18n('size') + ': ' + (getSize ? spinner : fm.formatSize(f.size)),
        );
        ...
        getSize = fm
          ...
          .done(function (data) {
            dialog
              ...
              .parent()
              .html(fm.i18n('size') + ': ' + data.formated);
          })
          .fail(function () {
            dialog
              .find('span.elfinder-spinner')
              .parent()
              .html(fm.i18n('size') + ': ' + fm.i18n('unknown'));
          })
          .always(function () {
            getSize = false;
          });
    ...
    toTrash = function (dfrd, targets, tHash) {
      ...
      if (itemCnt > maxCnt) {
        self.confirm(dfrd, targets, self.files(targets), null, [
          fm.i18n('tooManyToTrash'),
        ]);
        return;
      }
      ...
      reqDfd
        .done(function () {
          ...
          if (cnt) {
              ...
              .done(function (data) {
                var margeRes = function (data, phash, reqData) {
                    ...
                    if (data.added && data.added.length) {
                      undo = function () {
                        var targets = [],
                          dirs = $.map(data.added, function (f) {
                            return f.mime === 'directory' ? f.hash : null;
                          });
                  ...
                  res = {},
                  hasNtf = function () {
                    return fm.ui.notify.children('.elfinder-notify-trash')
                      .length;
                  },
                  hashes,
                  tm,
                              ...
                              if (err.length > 1) {
                                if (res.removed || res.removed.length) {
                                  hashes = $.grep(targets, function (h) {
                                    return $.inArray(h, res.removed) === -1
                                      ? true
                                      : false;
                                  });
                                }
            ...
            dfrd.reject([
              'error',
              'The folder hierarchy to be deleting can not be determined.',
            ]);
            fm.unlockfiles({files: targets});
          }
        })
    ...
    getTHash = function (targets) {
      ...
      if (targets && targets.length) {
        if (targets.length > 1 && fm.searchStatus.state === 2) {
          ...
          if (
            !$.grep(targets, function (h) {
              return h.indexOf(root1st) !== 0 ? true : false;
            }).length
          ) {
    ...
    self.change(function () {
      ...
      self.button &&
        self.button
          ...
          [self.value === 'trash' ? 'addClass' : 'removeClass'](
  ...
  this.getstate = function (select) {
    ...
    return sel.length &&
      $.grep(sel, function (h) {
        var f = fm.file(h);
        return f && !f.locked && !fm.isRoot(f) ? true : false;
      }).length == sel.length
      ? 0
      : -1;
  };
      ...
      dfrd = $.Deferred()
        ...
        .done(function (data) {
          !opts.quiet &&
            ...
            data.removed &&
            data.removed.length &&
            fm.trigger('playsound', {soundFile: 'rm.wav'});
        }),
    ...
    if (dfrd.state() === 'pending') {
      ...
      if (
        ...
        (self.event &&
          ...
          self.event.originalEvent.shiftKey)
      ) {
        tHash = '';
  ...
  fm.bind('select contextmenucreate closecontextmenu', function (e) {
    var targets =
      (e.data ? e.data.selected || e.data.targets : null) || fm.selected();
    if (targets && targets.length) {
      self.update(
        void 0,
        (targets ? getTHash(targets) : fm.option('trashHash')) ? 'trash' : 'rm',
      );
    }
  });
};
