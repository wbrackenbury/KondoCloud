...
elFinder.prototype.commands.rename = function () {
    ...
    batchRename = function () {
      var sel = fm.selected(),
        ...
        mkChk = function (node, label) {
          return $(
            '<label class="elfinder-rename-batch-checks">' +
              fm.i18n(label) +
              '</label>',
          ).prepend(node);
        },
  ...
  this.getstate = function (select) {
        ...
        cnt ===
          $.grep(sel, function (f) {
            if (
              ...
              !fm.isRoot(f) &&
              (mime === f.mime ||
                ext === fm.splitFileExtention(f.name)[1].toLowerCase())
            ) {
              return true;
            } else {
              brk && (brk = true);
    ...
    if (
      !isRoot &&
      state === 0 &&
      fm.option('disabledFlip', sel[0].hash)['rename']
    ) {
      state = -1;
    }
  ...
  this.exec = function (hashes, cOpts) {
    var cwd = fm.getUI('cwd'),
      sel = hashes ||
        (fm.selected().length ? fm.selected() : false) || [fm.cwd().hash],
      cnt = sel.length,
      ...
      type =
        opts._currentType === 'navbar' || opts._currentType === 'files'
          ? opts._currentType
          : incwd
          ? 'navbar'
          : 'files',
      ...
      blur = function (e) {
        ...
        if (!inError && pnode.length) {
          ...
          if (name === '.' || name === '..' || !valid) {
            inError = true;
            fm.error(
              file.mime === 'directory' ? 'errInvDirname' : 'errInvName',
              {
                modal: true,
                close: function () {
          ...
          if (cnt === 1) {
            ...
            fm.confirm({
              ...
              text: [
                'renameMultiple',
                cnt,
                getHint(name, [file.hash].concat(sel)),
              ],
              accept: {
                label: 'btnYes',
                callback: req,
      ...
      select = function () {
        ...
        input[0].setSelectionRange &&
          input[0].setSelectionRange(0, name.length);
      },
      node = navbar
    ...
    fm.one('select', function () {
      input.parent().length &&
        file &&
        $.inArray(file.hash, fm.selected()) === -1 &&
        input.trigger('blur');
    });
