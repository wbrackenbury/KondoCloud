...
elFinder.prototype.commands.search = function () {
  ...
  this.exec = function (
    ...
    if (typeof q == 'string' && q) {
      ...
      if (mime) {
        ...
        if (onlyMimes.length) {
          mime = $.map(mime, function (m) {
            m = $.trim(m);
            return m &&
              ($.inArray(m, onlyMimes) !== -1 ||
                $.grep(onlyMimes, function (om) {
                  return m.indexOf(om) === 0 ? true : false;
                }).length)
              ? m
              : null;
          });
      ...
      fm.trigger(
        'searchstart',
        setType({query: q, target: target, mimes: mime}),
      );
      ...
      if (!onlyMimes.length || mime.length) {
        if (target === '' && fm.api >= 2.1) {
            ...
            reqDef.push(
              fm.request({
                ...
                notify: {
                  type: 'search',
                  cnt: 1,
                  hideCnt: reqDef.length ? false : true,
                },
                cancel: true,
                preventDone: true,
              }),
          ...
          if (
            target !== '' &&
            fm.api >= 2.1 &&
            Object.keys(fm.leafRoots).length
          ) {
    ...
    fm.getUI('toolbar')
      .find('.' + fm.res('class', 'searchbtn') + ' :text')
      .trigger('focus');
