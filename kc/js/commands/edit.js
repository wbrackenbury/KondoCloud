      ...
      return $.grep(files, function (file) {
        ...
        res =
          ...
          (allowAll ||
            ...
            $.inArray(file.mime, cnt === 1 ? mimesSingle : mimes) !== -1) &&
          (!self.onlyMimes.length ||
            $.inArray(file.mime, self.onlyMimes) !== -1) &&
          (cnt === 1 ||
            (file.mime === mime &&
      ...
      if (!ta) {
        ...
        (function () {
          ...
          if (
            !ta.editor ||
            !ta.editor.info ||
            ta.editor.info.useTextAreaEvent
          ) {
              ...
              fm.confirm({
                ...
                optionsCallback: function (options) {
                  options.create = function () {
                    var base = $(
                        '<div class="elfinder-dialog-confirm-encoding"/>',
                      ),
    ...
    setEditors = function (file, cnt) {
      var mimeMatch = function (fileMime, editorMimes) {
          if (!editorMimes) {
            ...
            if (
              editorMimes[0] === '*' ||
              $.inArray(fileMime, editorMimes) !== -1
            ) {
              return true;
            }
            var i, l;
    ...
    getEditor()
      .done(function (editor) {
        while ((file = files.shift())) {
          list.push(
            edit(file, void 0, editor).fail(function (error) {
              error && fm.error(error);
            }),
          );
        }
