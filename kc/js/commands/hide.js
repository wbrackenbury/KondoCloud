...
elFinder.prototype.commands.hide = function () {
  ...
  this.fm.bind('select contextmenucreate closecontextmenu', function (e, fm) {
    ...
    if (
      !sel.length ||
      (((e.type !== 'contextmenucreate' && sOrigin !== 'navbar') ||
        cMenuType === 'cwd') &&
        sel[0] === fm.cwd().hash)
    ) {
    ...
    if (e.type !== 'closecontextmenu') {
      self.update(
        cMenuType === 'cwd' ? (hideCnt ? 0 : -1) : void 0,
        self.title,
      );
    } else {
      cMenuType = '';
  ...
  this.getstate = function (sel) {
    return (cMenuType !== 'cwd' && (sel || this.fm.selected()).length) ||
      hideCnt
      ? 0
      : -1;
  };
  ...
  this.exec = function (hashes, opts) {
    ...
    if (
      opts._currentType === 'shortcut' ||
      !items.length ||
      (opts._currentType !== 'navbar' &&
        sOrigin !== 'navbar' &&
        items[0] === fm.cwd().hash)
    ) {
      if (hideData.show) {
