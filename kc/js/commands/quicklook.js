...
(elFinder.prototype.commands.quicklook = function () {
  ...
  var self = this,
    ...
    openedCss = function () {
      ...
      return {
        ...
        top: parseInt(
          (win.height() - h - 60) / 2 +
            (contain ? 0 : win.scrollTop() - elf.top)
        ),
        left: parseInt(
          (win.width() - w) / 2 + (contain ? 0 : win.scrollLeft() - elf.left)
        ),
      };
    },
    mediaNode = {},
    support = function (codec, name) {
      var node = name || codec.substr(0, codec.indexOf("/")),
        media = mediaNode[node]
          ? mediaNode[node]
          : (mediaNode[node] = document.createElement(node)),
        value = false;
    ...
    fsicon = $('<div class="' + navicon + " " + navicon + '-fullscreen"/>').on(
      "click touchstart",
      function (e) {
        if (navmove) {
          return;
        ...
        if (full) {
          navStyle = "";
          navShow();
          win.toggleClass(fullscreen).css(win.data("position"));
          ...
          $(window).on(self.resize, resize).trigger(self.resize);

          cover
            .on(coverEv, function (e) {
              if (!navdrag) {
                if (e.type === "mousemove" || e.type === "touchstart") {
                  ...
                  navtm = setTimeout(function () {
                    if (
                      fm.UA.Mobile ||
                      navbar.parent().find(".elfinder-quicklook-navbar:hover")
                        .length < 1
                    ) {
        ...
        if (fm.UA.Mobile) {
          ...
          navbar.attr("style", navStyle).draggable(
            full
              ? "destroy"
              : {
                  start: function () {
    ...
    prev = $('<div class="' + navicon + " " + navicon + '-prev"/>').on(
      "click touchstart",
      function (e) {
        !navmove && navtrigger(leftKey);
        return false;
      }
    ),
    next = $('<div class="' + navicon + " " + navicon + '-next"/>').on(
      "click touchstart",
      function (e) {
        !navmove && navtrigger(rightKey);
        return false;
      }
    ),
    navbar = $('<div class="elfinder-quicklook-navbar"/>')
      ...
      .append(
        $('<div class="' + navicon + " " + navicon + '-close"/>').on(
          "click touchstart",
          function (e) {
            !navmove && self.window.trigger("close");
            return false;
          }
        )
      ),
    titleClose = $(
      '<span class="ui-front ui-icon elfinder-icon-close ui-icon-closethick"/>'
    ).on("mousedown", function (e) {
      e.stopPropagation();
      self.window.trigger("close");
    }),
    titleDock = $(
      '<span class="ui-front ui-icon elfinder-icon-minimize ui-icon-minusthick"/>'
    ).on("mousedown", function (e) {
      e.stopPropagation();
      if (!self.docked()) {
  ...
  this.info = $("<div/>").addClass(infocls).append(icon).append(info);
      ...
      var preview = self.preview,
        ...
        update = function () {
          ...
          if (
            file.hash ===
            fm.cwdId2Hash(
              cwd.find("[id]:not(.elfinder-cwd-parent):first").attr("id")
            )
          ) {
          ...
          info.html(
            tpl.replace(/\{value\}/, name) +
              ...
              tpl.replace(
                /\{value\}/,
                getSizeHashes.length ? spinner : fm.formatSize(file.size)
              ) +
              tpl.replace(
                /\{value\}/,
                fm.i18n("modify") + ": " + fm.formatDate(file)
              )
          );

          if (getSizeHashes.length) {
            getSize = fm
              .getSize(getSizeHashes)
              .done(function (data) {
          ...
          icon.addClass(
            "elfinder-cwd-icon ui-corner-all " + fm.mime2class(file.mime)
          );
  ...
  this.window = $(
    '<div class="ui-front ui-helper-reset ui-widget elfinder-quicklook touch-punch" style="position:absolute"/>'
  )
    .hide()
    ...
    .append(
      $(
        ...
        $(
          '<span class="ui-widget-header ui-dialog-titlebar-close ui-corner-all elfinder-titlebar-button elfinder-quicklook-titlebar-icon' +
            (platformWin ? " elfinder-titlebar-button-right" : "") +
            '"/>'
        ).append(titleClose, titleDock),
        title
      ),
      this.preview,
      self.info.hide(),
    ...
    .on("close", function (e, dfd) {
      ...
      if (self.opened()) {
        ...
        if (!self.docked()) {
          ...
          dockedNode = fm.getUI("navdock").data("removeNode")(
            self.window.attr("id"),
            "detach"
          );
          close(dockedhidden);
    ...
    .on("navdockin", function (e, data) {
      ...
      navbar.hide();
      titleDock.toggleClass(
        "ui-icon-plusthick ui-icon-minusthick elfinder-icon-full elfinder-icon-minimize"
      );
  ...
  this.handlers = {
    ...
    error: function () {
      self.window.is(":visible") && self.window.trigger("close");
    },
  ...
  this.support = {
    ...
    video: {
      ...
      m3u8:
        support("application/x-mpegURL", "video") ||
        support("application/vnd.apple.mpegURL", "video"),
    ...
    fm.one("load", function () {
      ...
      $(document).on("keydown." + fm.namespace, function (e) {
        e.keyCode == $.ui.keyCode.ESCAPE &&
          self.opened() &&
          !self.docked() &&
          win.hasClass("elfinder-frontmost") &&
          win.trigger("close");
      });
      ...
      preview.on(evUpdate, function (e) {
        ...
        if ((file = e.file)) {
          ...
          if (file.mime !== "directory") {
            if (
              parseInt(file.size) ||
              file.mime.match(o.mimeRegexNotEmptyCheck)
            ) {
              // set current dispInlineRegex
              self.dispInlineRegex = cwdDispInlineRegex;
              if (serach || fm.optionsByHashes[hash]) {
                try {
                  self.dispInlineRegex = new RegExp(
                    fm.option("dispInlineRegex", hash),
                    "i"
                  );
                } catch (e) {
                  try {
                    self.dispInlineRegex = new RegExp(
                      !fm.isRoot(file)
                        ? fm.option("dispInlineRegex", file.phash)
                        : fm.options.dispInlineRegex,
                      "i"
                    );
                  } catch (e) {
