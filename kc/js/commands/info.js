...
(elFinder.prototype.commands.info = function () {
  ...
  this.tpl = {
    main:
      '<div class="ui-helper-clearfix elfinder-info-title {dirclass}"><span class="elfinder-cwd-icon {class} ui-corner-all"{style}/>{title}</div><table class="elfinder-info-tb">{content}</table>',
    itemTitle:
  ...
  this.exec = function (hashes) {
    ...
    if (cnt === 1) {
          ...
          href &&
            content.push(
              row
                ...
                .replace(
                  v,
                  '<a href="' + href + '" target="_blank">' + name_esc + '</a>',
                ),
            );
        }
      }
        ...
        if (file.dim) {
          ...
          if (file.width && file.height) {
            content.push(
              row
                .replace(l, msg.dim)
                .replace(v, file.width + 'x' + file.height),
            );
          } else {
            content.push(
              row
      ...
      if (
        ...
        getHashAlgorisms = [];
        $.each(
          fm.storage('hashchekcer') || o.showHashAlgorisms,
          function (i, n) {
            if (!file[n]) {
              content.push(
                row
        ...
        reqs.push(
          fm
            .getContentsHashes(file.hash, getHashAlgorisms)
