...
elFinder.prototype.commands.sort = function () {
  ...
  var self = this,
    ...
    setVar = function () {
      ...
      $.each(fm.sortRules, function (name, value) {
        if (fm.sorters[name]) {
          var arr =
            name === fm.sortType ? (fm.sortOrder === 'asc' ? 'n' : 's') : '';
          self.variants.push([
            name,
            (arr
              ? '<span class="ui-icon ui-icon-arrowthick-1-' + arr + '"></span>'
              : '') +
              '&nbsp;' +
              fm.i18n('sort' + name),
          ]);
    ...
    updateContextmenu = function () {
      ...
      if (cm.is(':visible')) {
        ...
        sub.children('div.elfinder-contextsubmenu-item').each(function () {
          ...
          if (
            name ===
            (i18Name.stick || (i18Name.stick = fm.i18n('sortFoldersFirst')))
          ) {
            ...
            name ===
            (i18Name.tree || (i18Name.tree = fm.i18n('sortAlsoTreeview')))
          ) {
            ...
            name ===
            (i18Name[fm.sortType] ||
              (i18Name[fm.sortType] = fm.i18n('sort' + fm.sortType)))
          ) {
  ...
  fm.bind('sortchange', setVar)
    .bind('sorterupdate', function () {
      setVar();
      fm.getUI()
        .children('.elfinder-button-sort-menu')
        .children('.elfinder-button-menu-item')
        .each(function () {
          var tgt = $(this),
            rel = tgt.attr('rel');
