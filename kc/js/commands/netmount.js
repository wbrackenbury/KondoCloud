          ...
          $.each(self.drivers, function () {
            ...
            if (d) {
              ...
              if (d.integrateInfo) {
                fm.trigger(
                  'helpIntegration',
                  Object.assign({cmd: 'netmount'}, d.integrateInfo),
                );
              }
            }
          });
  ...
  this.exec = function () {
    var fm = self.fm,
      ...
      create = function () {
        var winFocus = function () {
            ...
            protocol: $('<select/>')
              .on('change', function (e, data) {
                ...
                requestAnimationFrame(function () {
                  content
                    .find('input:text.elfinder-tabstop:visible:first')
                    .trigger('focus');
                });
              })
          ...
          doMount = function () {
            var protocol = inputs.protocol.val(),
              ...
              cur = o[protocol];
            $.each(
              content.find('input.elfinder-netmount-inputs-' + protocol),
              function (name, input) {
                var val, elm;
                elm = $(input);
          ...
          form = $('<form autocomplete="off"/>').on(
            ...
            function (e) {
              ...
              if (e.keyCode === $.ui.keyCode.ENTER) {
                $.each(
                  form.find('input:visible:not(.elfinder-input-optional)'),
                  function () {
                    if ($(this).val() === '') {
                      comp = false;
                      next = $(this);
        ...
        content = $(
          '<table class="elfinder-info-tb elfinder-netmount-tb"/>',
        ).append(
          $('<tr/>')
            .append($('<td>' + fm.i18n('protocol') + '</td>'))
            .append($('<td/>').append(inputs.protocol)),
        );

        $.each(self.drivers, function (i, protocol) {
          if (o[protocol]) {
            inputs.protocol.append(
              '<option value="' +
                protocol +
                '">' +
                fm.i18n(o[protocol].name || protocol) +
                '</option>',
            );
            $.each(o[protocol].inputs, function (name, input) {
              ...
              if (input.attr('type') != 'hidden') {
                input.addClass(
                  'ui-corner-all elfinder-netmount-inputs-' + protocol,
                );
                content.append(
                  $('<tr/>')
                    .addClass(
                      'elfinder-netmount-tr elfinder-netmount-tr-' + protocol,
                    )
                    .append($('<td>' + fm.i18n(name) + '</td>'))
                ...
                $.when(requests)
                  .done(function () {
                    fm.request({
                      ...
                      .done(function (data) {
                        drive.volumeid &&
                          delete fm.volumeExpires[drive.volumeid];
                        dfrd.resolve();
                      });
                  })
                  .fail(function (error) {
                        ...
                        requests.push(
                          fm
                            ...
                            .done(function (data) {
                              if (data.removed) {
                                d.volumeid &&
                                  delete fm.volumeExpires[d.volumeid];
