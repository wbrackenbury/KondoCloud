elFinder.prototype.commands.quicklook.plugins = [
  ...
  function (ql) {
    ...
    preview.on(ql.evUpdate, function (e) {
          ...
          preview
            .on('changesize', function () {
              ...
              img
                .width(w)
                .height(h)
                .css('margin-top', h < ph ? Math.floor((ph - h) / 2) : 0);
            })
            .trigger('changesize');
      ...
      if (flipMime[file.mime] && ql.dispInlineRegex.test(file.mime)) {
        ...
        loading = $(
          '<div class="elfinder-quicklook-info-data"><span class="elfinder-spinner-text">' +
            fm.i18n('nowLoading') +
            '</span><span class="elfinder-spinner"/></div>',
  ...
  function (ql) {
    ...
    preview.on(ql.evUpdate, function (e) {
      ...
      if (
        ...
        fm.options.cdns.psd &&
        !fm.UA.ltIE10 &&
        ql.dispInlineRegex.test(file.mime)
      ) {
        // this is our file - stop event propagation
        e.stopImmediatePropagation();
        ...
        if (PSD) {
          ...
          window.require = null;
          window.define = null;
          fm.loadScript([fm.options.cdns.psd], function () {
            PSD = require('psd');
  ...
  function (ql) {
    ...
    preview.on(ql.evUpdate, function (e) {
      ...
      if (
        mimes[file.mime] &&
        ql.dispInlineRegex.test(file.mime) &&
        (!ql.options.getSizeMax || file.size <= ql.options.getSizeMax)
      ) {
        e.stopImmediatePropagation();
  ...
  function (ql) {
    ...
    if (
      (fm.UA.Safari && fm.OS === 'mac' && !fm.UA.iOS) ||
      fm.UA.IE ||
      fm.UA.Firefox
    ) {
      active = true;
    ...
    if (active) {
      if (
        typeof ql.options.pdfToolbar !== 'undefined' &&
        !ql.options.pdfToolbar
      ) {
        urlhash = '#toolbar=0';
      }
      preview.on(ql.evUpdate, function (e) {
        ...
        if (
          ...
          $(
            '<object class="elfinder-quicklook-preview-pdf" data="' +
              fm.openUrl(file.hash) +
              urlhash +
        ...
        if (file.mime === mime && ql.dispInlineRegex.test(file.mime)) {
          ...
          node = $(
            '<embed class="elfinder-quicklook-preview-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" src="' +
              fm.openUrl(file.hash) +
              '" quality="high" type="application/x-shockwave-flash" wmode="transparent" />',
          ).appendTo(preview);
        }
      });
  },
  ...
  function (ql) {
    ...
    var fm = ql.fm,
      ...
      autoplay,
      controlsList =
        typeof ql.options.mediaControlsList === 'string' &&
        ql.options.mediaControlsList
          ? ' controlsList="' + fm.escape(ql.options.mediaControlsList) + '"'
          : '',
      setNavi = function () {
        navi.css(
          'bottom',
          win.hasClass('elfinder-quicklook-fullscreen') ? '50px' : '',
        );
      },
      getNode = function (src, hash) {
        return $(
          '<audio class="elfinder-quicklook-preview-audio ui-front" controls' +
            controlsList +
            ' preload="auto" autobuffer><source src="' +
            src +
            '" /></audio>',
        )
    ...
    preview
      .on(ql.evUpdate, function (e) {
        ...
        if (
          mimes[file.mime] &&
          ql.dispInlineRegex.test(file.mime) &&
          ((html5 = ql.support.audio[type]) || type === 'amr')
        ) {
  ...
  function (ql) {
    ...
    var fm = ql.fm,
      ...
      render = function (file, opts) {
        ...
        ql.hideinfo();
        node = $(
          '<video class="elfinder-quicklook-preview-video" controls' +
            controlsList +
        ...
        node[0].addEventListener(
          ...
          function (e) {
            if (
              opts.src &&
              fm.convAbsUrl(opts.src) === fm.convAbsUrl(e.target.src)
            ) {
              ++err;
              errTm();
            }
    ...
    preview
      .on(ql.evUpdate, function (e) {
        ...
        if (
          mimes[mime] &&
          ql.dispInlineRegex.test(
            file.mime,
          ) /*&& (((type === 'm3u8' || (type === 'mpd' && !fm.UA.iOS) || type === 'flv') && !fm.UA.ltIE10) || ql.support.video[type])*/
        ) {
          autoplay = ql.autoPlay();
  ...
  function (ql) {
    ...
    $.each(navigator.plugins, function (i, plugins) {
      $.each(plugins, function (i, plugin) {
        (plugin.type.indexOf('audio/') === 0 ||
          plugin.type.indexOf('video/') === 0) &&
  ...
  function (ql) {
    ...
    var fm = ql.fm,
      ...
      unzipFiles = function () {
        ...
        for (i = 0, il = fileHeaderList.length; i < il; ++i) {
          ...
          filenameList[i] =
            fileHeaderList[i].filename +
            (fileHeaderList[i].J
              ? ' (' + fm.formatSize(fileHeaderList[i].J) + ')'
              : '');
        }
        return filenameList;
      },
    ...
    if (
      window.Uint8Array &&
      window.DataView &&
      fm.options.cdns.zlibUnzip &&
      fm.options.cdns.zlibGunzip
    ) {
      preview.on(ql.evUpdate, function (e) {
        var file = e.file,
        ...
        if (
          mimes[file.mime] &&
          (isTar ||
            ((typeof Zlib === 'undefined' || Zlib) &&
              (file.mime === 'application/zip' ||
                file.mime === 'application/x-gzip')))
        ) {
          var jqxhr,
            loading,
            url,
            ...
            makeList = function (filenames) {
              ...
              if (filenames && filenames.length) {
                ...
                header =
                  ...
                  fm.escape(file.mime) +
                  '</strong> (' +
                  fm.formatSize(file.size) +
                  ')' +
                  '<hr/>';
                doc = $(
                  '<div class="elfinder-quicklook-preview-archive-wrapper">' +
                    header +
                    '<pre class="elfinder-quicklook-preview-text">' +
                  ...
                  .on('touchstart', function (e) {
                    if (
                      $(this)[
                        'scroll' + (fm.direction === 'ltr' ? 'Right' : 'Left')
                      ]() > 5
                    ) {
  ...
  function (ql) {
    ...
    preview.on(ql.evUpdate, function (e) {
      var file = e.file;
      if (
        mimes[file.mime.toLowerCase()] &&
        !fm.option('onetimeUrl', file.hash)
      ) {
        var win = ql.window,
          loading,
          url;
        ...
        if (file.url !== '' && file.url != '1') {
          ...
          node = $(
            '<iframe class="elfinder-quicklook-preview-iframe" scrolling="no"/>',
          )
            .css('background-color', 'transparent')
            .appendTo(preview)
            .on('load', function () {
            ...
            .attr(
              'src',
              '//sharecad.org/cadframe/load?url=' + encodeURIComponent(url),
            );
    ...
    if (ql.options.googleMapsApiKey) {
      ...
      preview.on(ql.evUpdate, function (e) {
        ...
        if (mapScr && mimes[file.mime.toLowerCase()]) {
          ...
          if (file.url !== '' && !getLink) {
            node = $('<div style="width:100%;height:100%;"/>').appendTo(
              preview,
            );
            preview.one('change', function () {
              node.remove();
  ...
  function (ql) {
    ...
    var fm = ql.fm,
      mimes = Object.assign(
        fm.arrayFlip(ql.options.googleDocsMimes || [], 'g'),
        fm.arrayFlip(ql.options.officeOnlineMimes || [], 'm'),
      ),
      preview = ql.preview,
      win = ql.window,
      navi = ql.navbar,
    ...
    if (ql.options.officeOnlineMimes.length) {
      ...
      ql.addIntegration({
        ...
        link:
          'https://products.office.com/office-online/view-office-documents-online',
      });
    }
        ...
        if (file.size <= 26214400 && (type = mimes[file.mime])) {
          var win = ql.window,
            setNavi = function () {
              navi.css(
                'bottom',
                win.hasClass('elfinder-quicklook-fullscreen')
                  ? navBottom[type]
                  : '',
              );
            },
          ...
          if (type === 'm') {
            if (
              (mLimits[ext] && file.size > mLimits[ext]) ||
              file.size > mLimits.other
            ) {
              type = 'g';
            }
          }
            ...
            fm.forExternalUrl(file.hash).done(function (url) {
              if (url) {
                ...
                node
                  ...
                  .attr(
                    'src',
                    'https://' + urls[type] + encodeURIComponent(url),
                  );
              } else {
                loading.remove();
                node.remove();
    ...
    var fm = ql.fm,
      ...
      prettify = function () {
        if (fm.options.cdns.prettify) {
          fm.loadScript([
            fm.options.cdns.prettify +
              (fm.options.cdns.prettify.match(/\?/) ? '&' : '?') +
              'autorun=false',
          ]);
          prettify = function () {
            return true;
    ...
    preview.on(ql.evUpdate, function (e) {
      ...
      if (
        ...
        jqxhr = fm
          ...
          .done(function (data) {
            var reg = new RegExp(
                '^(data:' + file.mime.replace(/([.+])/g, '\\$1') + ';base64,)',
                'i',
              ),
              text = data.content,
              part,
            ...
            node = $(
              '<div class="elfinder-quicklook-preview-text-wrapper"><pre class="elfinder-quicklook-preview-text prettyprint"></pre></div>',
            );

            if (more) {
              node.append(
                $(
                  '<div class="elfinder-quicklook-preview-charsleft"><hr/><span>' +
                    fm.i18n('charsLeft', fm.toLocaleString(more)) +
                    '</span></div>',
                ).on('click', function () {
