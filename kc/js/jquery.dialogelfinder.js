  ...
  if ($.isPlainObject(opts)) {
    this.not('.elfinder').each(function () {
      ...
      var node = $(this),
        ...
        button = $(
          '<a href="#" class="dialogelfinder-drag-close ui-corner-all"><span class="ui-icon ui-icon-closethick"> </span></a>',
        )
          .appendTo(toolbar)
          .on('click', function (e) {
            e.preventDefault();
      ...
      elfinder = node
        .addClass('elfinder dialogelfinder touch-punch')
      ...
      node
        .width(parseInt(node.width()) || 840) // fix width if set to "auto"
    ...
    if (opts === 'open') {
      ...
      pos = node.data(position) || {
        top: parseInt(
          $(document).scrollTop() +
            ($(window).height() < node.height()
              ? 2
              : ($(window).height() - node.height()) / 2),
        ),
        left: parseInt(
          $(document).scrollLeft() +
            ($(window).width() < node.width()
              ? 2
              : ($(window).width() - node.width()) / 2),
        ),
      };
