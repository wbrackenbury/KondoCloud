...
var elFinder = function (elm, opts, bootCallback) {
  ...
  var self = this,
    ...
    recommends = {},
    /**
     * Preview recommendations provided to the user
     *
     * @type Array
     **/
    findrecommends = [],
    /**
     * Log of recommendations provided to the user
     *
     * @type Array
     **/
    invrecommends = [],
    ...
    open = function (data) {
      ...
      if (self.api >= 2.1) {
        ...
        self.commandMap =
          data.options.uiCmdMap && Object.keys(data.options.uiCmdMap).length ? data.options.uiCmdMap : {};
        ...
        setSorter = function (file) {
          ...
          $.each(self.sortRules, function (key) {
            if (
              ...
              (key === "mode" && typeof f.perm !== "undefined")
            ) {
              sorters.push(key);
            }
          });
  ...
  if (self.UA.Mobile) {
    $(window)
      .on("orientationchange." + namespace, function () {
  ...
  if (opts.uiOptions) {
    if (opts.uiOptions.toolbar && Array.isArray(opts.uiOptions.toolbar)) {
      if ($.isPlainObject(opts.uiOptions.toolbar[opts.uiOptions.toolbar.length - 1])) {
        self.options.uiOptions.toolbarExtra = Object.assign(
          self.options.uiOptions.toolbarExtra || {},
          opts.uiOptions.toolbar.pop()
        );
      }
    }
  }
  ...
  if (this.options.cssAutoLoad) {
    (function () {
      var baseUrl = self.baseUrl,
        myCss = $('head > link[href$="css/elfinder.min.css"],link[href$="css/elfinder.full.css"]:first')
          .length,
        rmTag = function () {
  ...
  (function () {
    if (typeof self.options.cors !== "undefined" && self.options.cors !== null) {
      ...
      if (
        selfHost !== parseUrl.host.replace(portReg(parseUrl.protocol), "") ||
        ...
        (parseUploadUrl &&
          (selfHost !== parseUploadUrl.host.replace(portReg(parseUploadUrl.protocol), "") ||
            (parseUploadUrl.protocol !== ":" &&
              parseUploadUrl.protocol !== "" &&
              selfProtocol !== parseUploadUrl.protocol)))
      ) {
        self.isCORS = true;
      }
    }
  ...
  this.droppable = {
    ...
    drop: function (e, ui) {
      ...
      if (
        ...
        ui.helper.data("namespace") !== namespace ||
        !self.insideWorkzone(e.pageX, e.pageY)
      ) {
        return false;
      }
    ...
    var file = files[hash],
      ...
      getUrl = function (url) {
        ...
        if (baseUrl) {
          return filter(
            baseUrl +
              $.map(self.path2array(hash), function (n) {
        ...
        return filter(
          self.options.url + (self.options.url.indexOf("?") === -1 ? "?" : "&") + $.param(params, true)
        );
      },
      baseUrl,
      res;
  ...
  this.request = function (opts) {
    var self = this,
      ...
      data = Object.assign(
        ...
        { mimes: o.onlyMimes },
        opts.data || opts,
        { cmd_state: self.options.openCallbackArgs.state } || {}
      ),
      // command name
      cmd = data.cmd,
       ...
       **/
        ...
        var clocktime = data.org_time;

        if (clocktime !== null) {
          $("#clock").text(`${clocktime} mins`);
          if (clocktime >= 30) {
            $("#study-submit-button").button("enable");
          }
        }
        ...
        if (isOpen) {
          ...
          if (data.recs) {
            self.updateRecs(data);
          }
          if (data.findrecs) {
            self.updateFindRecs(data);
          }
        }
                ...
                var targets = response[actionTarget],
                  ...
                  makeToast = function (t) {
                    ...
                    if (
                      ...
                      (msg = t.action.msg) &&
                      (cmd = t.action.cmd) &&
                      (!t.action.cwdNot || t.action.cwdNot !== self.cwd().hash)
                    ) {
                      done = t.action.done;
                      data = t.action.data;
                      node = $("<div/>").append(
          ...
          .fail(function (error, xhr, response) {
            ...
            if (isOpen) {
              ...
              openDir &&
                openDir.volumeid &&
                self.isRoot(openDir) &&
                delete self.volumeExpires[openDir.volumeid];
            }
  ...
  this.tutorial = function () {
    var example_html = `
        <div class="elfinder-new-rec" id="big-ex">
          <div class="elfinder-rec-button-container">
            <div class="elfinder-new-butt-container">
              <div tooltip="Accept recommendation" class="elfinder-rec-butts ui-state-default elfinder-button elfinder-acc-butt">
                <span class="elfinder-button-icon-accept"></span>
              </div>
            </div>
            <div class="elfinder-rec-del-sec">
              <div class="elfinder-rec-butts ui-state-default elfinder-rec-small-button elfinder-rej-butt">
                <div class="elfinder-button-icon-exit"></div>
              </div>
            </div>
          </div>
          <div class="elfinder-new-rec-content">
            <div class="elfinder-new-rec-row">
              <span class="elfinder-new-rec-row-left-label" id="examplespan" >Move:</span>
              <span class="elfinder-name-hover" id="examplehover" class="elfinder-name-hover">Ponemon_Data_Breach.PDF</span>
            </div>
            <div class="elfinder-new-rec-row">
              <span class="elfinder-new-rec-row-left-label">From:</span>
              <span class="elfinder-name-hover" title="/root">/root</span>
            </div>
            <div class="elfinder-new-rec-row">
              <span class="elfinder-new-rec-row-left-label">To:</span>
              <span class="elfinder-name-hover" title="/root/NewFolder">/root/NewFolder</span>
            </div>
          </div>
        </div>
    `;
    ...
    expl.html("Because you moved <b>data_breach.xlsx</b> to this location");
    $("#moveheader").append($(example_html));
    $("#moveheader .empty-recs-msg").css("display", "none");
    $("body").append(expl);

    expl.css({
      top: $("#examplespan").offset().top + topoffset,
      left: $("#examplespan").offset().left + leftoffset,
      display: "none",
    });

    $("#big-ex").css("display", "none");

    const driver = new Driver({
      allowClose: false,
      onReset: function () {
        $("#big-ex").remove();
        $("#moveheader .empty-recs-msg").css("display", "block");
    ...
    driver.defineSteps([
      {
        element: ".elfinder-workzone",
        popover: {
          title: "Welcome",
          description:
            "Welcome! This is the interface you will be using to organize your account. It should have most of the same features you're used to from your favorite file browser. It also, though, has a few other features that we've added.",
          position: "right",
        },
      },
      {
        element: ".elfinder-recommend",
        popover: {
          title: "Recommendations",
          description:
            "This box will occasionally recommend file management actions you might find useful. For example, if you move a file that our program thinks is similar to another file our program may offer you an easy way to move that file to the same place.",
          position: "right",
        },
        onNext: function () {
          $("#big-ex").css("display", "flex");
        },
      },
      {
        element: "#big-ex",
        popover: {
          title: "Recommendations",
          description:
            "A recommendation might look something like this. Our program uses the information about your files that we analyzed from Part 1 of this study, like the similarity between filenames, to make these recommendations.",
          position: "right",
        },
        onPrevious: function () {
          $("#big-ex").css("display", "none");
      ...
      },
      {
        ...
        popover: {
          title: "Recommendations",
          description:
            "If you're curious about why our program recommended an action, hover over the recommendation...",
          position: "right",
        },
        onNext: function () {
          $("#rec-hover-tool").css("display", "block");
        },
      },
      {
        element: "#rec-hover-tool",
        popover: {
          title: "Recommendations",
          description: "... to reveal the explanation!",
          position: "right",
        },
        onNext: function () {
          $("#rec-hover-tool").css("display", "none");
        },
        onPrevious: function () {
          $("#rec-hover-tool").css("display", "none");
        },
      ...
      {
        element: "#examplehover",
        popover: {
          title: "Recommendations",
          description:
            "You can also click the name of the file to navigate directly to where the file is in your file tree.",
          position: "right",
        },
        onPrevious: function () {
          $("#rec-hover-tool").css("display", "block");
      ...
      },
      {
        ...
        popover: {
          title: "Recommendations",
          description:
            "If you like the recommendation, and want to perform the action it suggests, just click the checkmark sign! ",
          position: "right",
        },
      },
      {
        element: "#tutorial-del",
        popover: {
          title: "Recommendations",
          description:
            "If you don't like the recommendation, though, you can click this 'X' to delete it, click the 'X' next to 'Clear All' to delete all of them, or just wait! If you take 5 actions (like moving a file) without accepting the recommendation, it will disappear.",
          position: "right",
        },
      },
      {
        ...
        popover: {
          ...
          description:
            "Like the box that may contain suggestions of files to move, this box may contain suggestions of files to delete.",
          position: "right",
        },
...

      {
        element: "#findheader",
        popover: {
          title: "Potentially Relevant Files",
          description:
            "And this one, like the others, may display items that you might be interested in looking at to remind you of their contents. These recommendations function like the ones for the other boxes. To view the file, you can click the filename in the recommendation, and it will take you to where the file is located.",
          position: "right",
        },
        stageBackground: "none",
      },
      {
        element: ".elfinder-recommend-accordion",
        popover: {
          title: "Log of recommendations",
          description:
            "If you accept a recommendation, but then decide you made a mistake, you can open the log by clicking, and click the undo button inside there!",
          position: "top",
        },
      },
      {
        element: "#study-instructions",
        popover: {
          title: "Review Instructions",
          description:
            "If you would like to review the instructions for the study, you may click this button.",
          position: "top",
        },
      },
      {
        element: "#study-submit-button",
        popover: {
          title: "Submit Organization",
          description:
            "Once you have finished organizing, you may click this button to move on to the next portion of the study. This button will become enabled after you have 30 minutes organizing.",
          position: "top",
        },
      },
      {
        ...
        popover: {
          title: "Instructions",
          description:
            "If you have any questions about the interface or the task, feel free to return to the study instructions at any time. Feel free to begin as soon as you click the 'Next' or 'Done' buttons.",
          position: "right",
        },
        onNext: function () {},
      },
    ...
    driver.defineSteps([
      {
        ...
        popover: {
          title: "Welcome",
          description:
            "Welcome! This is the interface you will be using to organize your account. It should have most of the same features you're used to from your favorite file browser.",
          position: "right",
        },
      },
      {
      ...
      },
      {
        ...
        popover: {
          title: "Submit Organization",
          description:
            "Once you have finished organizing, you may click this button to move on to the next portion of the study.",
          position: "top",
        },
      },
      {
        ...
        popover: {
          ...
          description:
            "If you have any questions about the interface or the task, feel free to return to the study instructions at any time. Feel free to begin as soon as you click Next or Close.",
          position: "right",
        },
      },
    ]);
  ...
  /**
   * Update recommendation objects based on data
   *
   * @param  Object  respose data object
   * @return void
   */

  this.updateRecs = function (data) {
    recommends = data.recs;
  };

  this.idComp = function (rid) {
    return function (e) {
      return e.id == rid;
    };
  };

  this.recsComp = function (r) {
    return function (e) {
      return r.phash == e.phash && e.action == r.action;
    };
  };

  /**
   * Update find recommendation objects based on data
   *
   * @param  Object  respose data object
   * @return void
   */

  this.updateFindRecs = function (data) {
    findrecommends = data.findrecs;
  };
  ...
  this.getFindRecs = function () {
    return findrecommends;
  };

  this.getInvRecs = function () {
    return invrecommends;
  };

  this.getRecStyle = function () {
    return self.options.recDisplay;
  };

  this.removeRec = function (id) {
    self.generalRemoveRec(id, false);
  };

  this.removeAndInvRec = function (id) {
    self.generalRemoveRec(id, true);
  };

  this.generalRemoveRec = function (id, alsoinv) {
    var infunc = function (rid) {
        return function (e) {
          return rid == e.id;
        };
      },
    ...
    for (var key of Object.keys(recommends)) {
      recs_arr = recommends[key];
      found = recs_arr.findIndex(infunc(id));

      if (found != -1) {
        deltd = recs_arr.splice(found, 1);
        if (alsoinv) {
          self.addInvRecs(deltd);
        }
      }
    }
  };

  this.addInvRecs = function (invrecs) {
    for (var rt of invrecs) {
      if (invrecommends.findIndex(self.recsComp(rt)) == -1) {
        rt.inv = true;
        invrecommends.unshift(rt);
      }
    }
  };

  this.invRec = function (id) {
    var infunc = self.idComp,
      i,
      rtemp,
      found,
      recs_arr,
    ...
    found = invrecommends.findIndex(infunc(id));
    deltd = invrecommends.splice(found, 1);

    for (var rec of deltd) {
      rec.is_redo = true;
    }
  ...
  this.removeFindRec = function (id) {
    var infunc = self.idComp,
      i,
      rtemp,
      found,
    ...
    found = findrecommends.findIndex(infunc(id));
    findrecommends.splice(found, 1);
  };

  /*
      Sort in reverse strength order
      */

  this.recStrengthComp = function (a, b) {
    return b.strength - a.strength;
  };

  this.recStrengthDecayComp = function (a, b) {
    return b.strength * b.num - a.strength * a.num;
  };
  ...
  this.exec = function (cmd, files, opts, dstHash) {
    ...
    dfrd =
      this._commands[cmd] && this.isCommandEnabled(cmd, dstHash)
    ...
    if (!(resType === "object" && dfrd.promise)) {
      self.debug(
        "warning",
        '"cmd.exec()" should be returned "$.Deferred" but cmd "' + cmd + '" returned "' + resType + '"'
      );
  ...
  this.toast = function (options) {
    return $('<div class="ui-front"/>')
      .appendTo(this.ui.toast)
      .elfindertoast(options || {}, this);
  };
      ...
      if ((mt = h.match(/^([0-9.]+)%$/))) {
        ...
        if (!heightBase.data("fitToBaseFunc")) {
          heightBase.data("fitToBaseFunc", function (e) {
            ...
            if (
              !node.hasClass("elfinder-fullscreen") &&
              (!self.UA.Mobile || heightBase.data("rotated") !== self.UA.Rotated)
            ) {
  ...
  this.toFront = function (target) {
    ...
    nodes.css("z-index", "");
    $(target)
      .addClass("ui-front elfinder-frontmost")
      .css("z-index", lastnode.css("z-index") + 1);
  };
  ...
  (function () {
    ...
    var orgStyle,
      ...
      setFuncObj = function () {
        ...
        funcObj =
          self.UA.Fullscreen &&
          (useFullscreen ? useFullscreen > 0 : self.options.commandsOptions.fullscreen.mode === "screen")
            ? {
      ...
      restoreStyle = function (elem) {
        if (orgStyle && orgStyle.elm == elem) {
          $(elem)
            .removeClass(cls + " " + clsN)
            .attr("style", orgStyle.style);
          orgStyle = null;
        }
      },
      resize = function (e) {
  ...
  (function () {
    var cls = "elfinder-maximized",
      ...
      toMax = function (elm) {
        elm
          .data("bodyOvf", $("body").css("overflow") || "")
  ...
  this.xhrUpload =
    (typeof XMLHttpRequestUpload != "undefined" || typeof XMLHttpRequestEventTarget != "undefined") &&
  ...
  (function (self) {
    var hashLibs = {
      ...
      shaCalc = function (arr, length) {
        ...
        try {
          sha = new hashLibs.jsSHA(
            "SHA" + (length.substr(0, 1) === "3" ? length : "-" + length),
            "ARRAYBUFFER"
          );
          job = self
            .asyncJob(function (buf) {
    ...
    if (self.options.cdns.jssha) {
      self.hashCheckers = self.hashCheckers.concat([
        "sha1",
        "sha224",
        "sha256",
        "sha384",
            ...
            .done(function (arrayBuffer) {
              var arr =
                  arrayBuffer instanceof ArrayBuffer && arrayBuffer.byteLength > 0
                    ? self.sliceArrayBuffer(arrayBuffer, 1048576)
                    : false,
                i;
              ...
              if (hashLibs.jsSHA) {
                $.each(
                  ["1", "224", "256", "384", "512", "3-224", "3-256", "3-384", "3-512", "ke128", "ke256"],
                  function (i, v) {
                    if (needs["sha" + v]) {
                      jobs.push(function () {
  ...
  this.enable(function () {
    if (
      ...
      self.api &&
      self.visible() &&
      self.ui.overlay.is(":hidden") &&
      !node.children(".elfinder-dialog." + self.res("class", "editing") + ":visible").length
    ) {
      enabled = true;
    ...
    .select(function (e) {
      ...
      if (unselects.length) {
        ...
        self.toast({
          mode: "warning",
          msg: self.i18n(["errMaxTargets", self.maxTargets]),
        });
      }
    })
    .error(function (e) {
      ...
      if (cnt < self.options.maxErrorDialogs) {
        ...
        self.dialog(
          '<span class="elfinder-dialog-icon elfinder-dialog-icon-error"/>' + self.i18n(e.data.error),
          opts
        );
      } else {
        ...
        if (counter.length) {
          counter
            .data("cnt", parseInt(counter.data("cnt")) + 1)
            .html(self.i18n(["moreErrors", counter.data("cnt")]));
        } else {
          counter = $(
            '<span class="elfinder-error-counter">' + self.i18n(["moreErrors", 1]) + "</span>"
          ).data("cnt", 1);
  ...
  if (true === this.options.sound) {
    this.bind("playsound", function (e) {
      ...
      play &&
        ...
        play != "no" &&
        $(beeper)
          .html('<source src="' + soundPath + file + '" type="audio/wav">')[0]
          .play();
    });
  }
  ...
  self.one("opendone", function () {
    ...
    self.options.useBrowserHistory &&
      $(window).on("popstate." + namespace, function (ev) {
    ...
    $(window)
      .on("resize." + namespace, function (e) {
        if (e.target === this) {
          tm && cancelAnimationFrame(tm);
      ...
      .on("beforeunload." + namespace, function (e) {
        ...
        if (!self.pauseUnloadCheck()) {
          if (node.is(":visible")) {
            if (
              self.ui.notify.children().length &&
              $.inArray("hasNotifyDialog", self.options.windowCloseConfirm) !== -1
            ) {
              msg = self.i18n("ntfsmth");
            } else if (
              node.find("." + self.res("class", "editing")).length &&
              $.inArray("editingFile", self.options.windowCloseConfirm) !== -1
            ) {
              msg = self.i18n("editingFile");
            } else if (
              (cnt = Object.keys(self.selected()).length) &&
              $.inArray("hasSelectedItem", self.options.windowCloseConfirm) !== -1
            ) {
              msg = self.i18n("hasSelected", "" + cnt);
            } else if (
              (cnt = Object.keys(self.clipboard()).length) &&
              $.inArray("hasClipboardData", self.options.windowCloseConfirm) !== -1
            ) {
    ...
    (function () {
      var lang = self.lang,
        ...
        dfd = $.Deferred().done(function () {
          ...
          i18n =
            self.lang === "en" ? self.i18["en"] : $.extend(true, {}, self.i18["en"], self.i18[self.lang]);
        });
  ...
  bootUp = function () {
    ...
    // check jquery ui
    if (
      !($.fn.selectable && $.fn.draggable && $.fn.droppable && $.fn.resizable && $.fn.button && $.fn.slider)
    ) {
    ...
    (function () {
      ...
      self.draggable = {
        ...
        stop: function (e, ui) {
          ...
          $(document).off(keyEvt);
          $(this).elfUiWidgetInstance("draggable") &&
        ...
        helper: function (e, ui) {
          var element = this.id ? $(this) : $(this).parents("[id]:first"),
            helper = $(
              '<div class="elfinder-drag-helper"><span class="elfinder-drag-helper-icon-status"/></div>'
            ),
            icon = function (f) {
              var mime = f.mime,
                i,
                tmb = self.tmb(f);
              i =
                '<div class="elfinder-cwd-icon elfinder-cwd-icon-drag ' +
                self.mime2class(mime) +
                ' ui-corner-all"/>';
              if (tmb) {
                i = $(i)
          ...
          $(document).on(keyEvt, function (e) {
            ...
            if (ctr !== chk) {
              ...
              if (helper.is(":visible") && helper.data("dropover") && !helper.data("droped")) {
                ...
                self.trigger(ctr ? "unlockfiles" : "lockfiles", {
                  files: hashes,
                  helper: helper,
                });
              }
    ...
    if (self.commands.getfile) {
      if (typeof self.options.getFileCallback == "function") {
        self.bind("dblclick", function (e) {
          ...
          self.request({ data: { cmd: "find", target: e.data.file } });
        });
        self
          .shortcut({
            pattern: "enter",
    ...
    $.each(self.commands, function (name, cmd) {
      ...
      if (
        $.isFunction(cmd) &&
        !self._commands[name] &&
        (cmd.prototype.forceLoad || $.inArray(name, self.options.commands) !== -1)
      ) {
    ...
    self.ui = {
      ...
      recommend: (function () {
        if (self.options.condition !== "reg_int") {
          $("<div/>").prependTo(node).elfinderrecommend(self);
        } else {
          $("<div/>").appendTo(node);
          $(".elfinder-workzone").width("100%");
        }
      ...
      navbar: $("<div/>")
        .appendTo(node)
        .elfindernavbar(self, self.options.uiOptions.navbar || {})
        .append($('<div class="elfinder-statusbar"/>')),
      statusbar: $(".elfinder-statusbar"),
      // statusbar: $(
      ...
      navdock: $("<div/>")
        .appendTo(node)
        .elfindernavdock(self, self.options.uiOptions.navdock || {}),
      // contextmenu
      ...
      cwd: $("<div/>")
        .appendTo(node)
        .elfindercwd(self, self.options.uiOptions.cwd || {}),
        ...
        node.on("touchstart touchmove touchend", function (e) {
          ...
          if (e.type === "touchstart") {
            ...
            if (navbar && lastX !== false) {
              ...
              if (
                (navbarMode === "navhide" && moveX > navbarW * 0.6) ||
      ...
      (function () {
        var isin = function (e) {
            return (
              ...
              $(e.target).closest("div.ui-dialog-content").length === 0
            );
          },
              ...
              if (!autoScrTm) {
                autoScrTm = requestAnimationFrame(function () {
                  ...
                  if ((autoUp = e.pageY < wzRect.top) || e.pageY > wzBottom2) {
                    ...
                    fn &&
                      self.autoScroll[fn](Math.pow(autoUp ? wzRect.top - e.pageY : e.pageY - wzBottom2, 1.3));
                  }
                  autoScrTm = null;
                });
              }
        ...
        node
          .on("dragenter", ".native-droppable", function (e) {
            if (e.originalEvent.dataTransfer) {
              ...
              if (
                !cwd ||
                (cwd.write && (!elfFrom || elfFrom !== (window.location.href + cwd.hash).toLowerCase()))
              ) {
                e.preventDefault();
                e.stopPropagation();
                $elm.data(ent, true);
          ...
          .on("drop", ".native-droppable", function (e) {
            if (e.originalEvent && e.originalEvent.dataTransfer) {
              ...
              if (e.currentTarget.id) {
                id = $elm.hasClass(navdir)
                  ? self.navId2Hash(e.currentTarget.id)
                  : self.cwdId2Hash(e.currentTarget.id);
              } else {
                id = self.cwd().hash;
              }
    ...
    self
      ...
      .request(
        (function () {
          if (self.options.openCallbackArgs.doOpen === "1") {
            return {
              data: {
                cmd: "open",
                target: self.options.openCallbackArgs.toOpen,
                init: 1,
                tree: 1,
              },
              preventDone: true,
    ...
    if (self.options.openCallbackArgs.doOpen === "1") {
      if (self.options.openCallbackArgs.toClick !== "0") {
        setTimeout(function () {
          $("#" + self.options.openCallbackArgs.toClick).trigger("click");
        }, 300);
      }
    }

    var footer = $(`<div class="experiment-footer"/>`);
    $("body").append(footer);

    /**
     * Create a button for the study row on the bottom of the application
     * @param {*} btype - string saying which button type it is
     * @returns
     */
    function studyButton(btype) {
      var cont = $("<div class='footer-button'></div>");
      var disabled = self.res("class", "disabled");
      var hover = self.res("class", "hover");
      var isInstruct = btype === "instruct";
      const popover = `<div class="study-instruct-popover">
        The study is not yet complete. You must first complete the full amount of time.
      </div>`;
      var outerbutton = $(
        `<div class="elfinder-button"
          id="${!isInstruct ? "study-submit-button" : "study-instructions"}">
          ${!isInstruct ? popover : ""}
          <span class="elfinder-button-text">${isInstruct ? "Instructions" : "Done Organizing"}</span>
          <span class="elfinder-button-icon elfinder-button-icon-${
            !isInstruct ? "submit" : "instruct"
          }"></span>
        </div>`
      )
        .on("mouseenter mouseleave", function (e) {
          !outerbutton.hasClass(disabled) &&
            outerbutton[e.type == "mouseleave" ? "removeClass" : "addClass"](hover);
        })
        .on("click", function (e) {
          if (btype === "instruct") {
            ...
            window.open(
              "/instructions",
              "window",
              "toolbar=no, menubar=no, resizable=yes, height=500, width=500"
            );
          } else if (btype === "submit") {
            self.confirm({
              title: "Finished organizing?",
              text:
                "Are you certain that you are finished organizing? You will not be able to return to this interface after continuing on.\nIf you are finished, please allow up to 30 seconds to be redirected.",
              accept: {
                label: "Finished",
                callback: function () {
                  // window.location.href = "/start_content";
                  window.location.href = "/group_q";
                },
              },
              cancel: {
                label: "btnCancel",
      ...
      cont.append(outerbutton);

      return cont;
    }
    var tut = window.localStorage.getItem("tutorial");

    var timetext = $(
      `<div class="experiment-footer-time-text">
        <span >Time spent organizing: </span>
      </div>`
    );
    var clock = $(
      `<div class="experiment-footer-time-readout">
        <span id="clock">0 mins</span> out of 30 mins
      </div>`
    );
    var clockbox = $(`<div class="experiment-footer-clockbox"></div>`);

    clockbox.append(timetext);
    clockbox.append(clock);
    var buttonContainer = $('<div class="experiment-footer-button-container"></div>');
    buttonContainer.append(studyButton("submit"));
    buttonContainer.append(studyButton("instruct"));
    footer.append(buttonContainer);
    footer.append(clockbox);

    // TESTING TUTORIAL FLAG!
    // self.tutorial();

    if (self.options.condition === "reg_int" && tut !== "done") {
      $(".elfinder-workzone").width("50%");
      self.plain_tutorial();
      $(".elfinder-workzone").width("100%");
      window.localStorage.setItem("tutorial", "done");
    }

    if (self.options.condition === "full_recs" && tut !== "done") {
      self.tutorial();
      window.localStorage.setItem("tutorial", "done");
    }

    $("#study-submit-button").button({ disabled: true });
  ...
  if (bootCallback && typeof bootCallback === "function") {
    ...
    bootCallback.call(node.get(0), self, {
      dfrdsBeforeBootup: dfrdsBeforeBootup,
    });
  }

  // call dfrdsBeforeBootup functions then boot up elFinder
  $.when
    .apply(null, dfrdsBeforeBootup)
    .done(function () {
      bootUp();
    })
    .fail(function (error) {
...
elFinder.prototype = {
  ...
  uploads: {
        ...
        check = function () {
          ...
          fm.request({
            data: {
              cmd: "ls",
              target: target,
              intersect: $.map(names, function (item) {
                return item.name;
              }),
            },
    ...
    checkFile: function (data, fm, target) {
      if (!!data.checked || data.type == "files") {
        ...
        if (data.type == "html") {
          var tmp = $("<html/>").append($.parseHTML(str.replace(/ src=/gi, " _elfsrc="))),
            atag;
          ...
          atag.each(function () {
            ...
            if ((text = $(this).text())) {
              ...
              if (
                loc.href &&
                loc.href.match(/^(?:ht|f)tp/i) &&
                (atag.length === 1 ||
                  !loc.pathname.match(/(?:\.html?|\/[^\/.]*)$/i) ||
                  $.trim(text).match(/\.[a-z0-9-]{1,10}$/i))
              ) {
    ...
    xhr: function (data, fm) {
      var self = fm ? fm : this,
        ...
        dfrd = $.Deferred()
          ...
          .always(function () {
            ...
            chunkMerge &&
              notifyElm.children(".elfinder-notify-chunkmerge").length &&
          ...
          if (xhr.status == 0) {
            if (abort) {
              ...
              if (
                !isDataType &&
                data.files &&
                $.grep(data.files, function (f) {
                  return !f.type && f.size === (self.UA.Safari ? 1802 : 0) ? true : false;
                }).length
              ) {
                dfrd.reject(["errAbort", "errFolderUpload"]);
              } else if (
                data.input &&
                $.grep(data.input.files, function (f) {
                  return !f.type && f.size === (self.UA.Safari ? 1802 : 0) ? true : false;
                }).length
              ) {
      ...
      var send = function (files, paths) {
        var size = 0,
          ...
          BYTES_PER_CHUNK = Math.min(
            (fm.uplMaxSize ? fm.uplMaxSize : 2097152) - 8190,
            fm.options.uploadMaxChunkSize
          ), // uplMaxSize margin 8kb or options.uploadMaxChunkSize
        ...
        if (!dataChecked && (isDataType || data.type == "files")) {
          ...
          for (i = 0; i < files.length; i++) {
            ...
            // file size check
            if (
              (maxFileSize && blobSize > maxFileSize) ||
              (!blobSlice && fm.uplMaxSize && blobSize > fm.uplMaxSize)
            ) {
            ...
            $.each(files, function (i, file) {
              ...
              if (excludes && file.name.match(excludes)) {
                ...
                if (relPath && $.inArray(relPath, mkdirs) === -1) {
                  ...
                  if (
                    idx !== -1 &&
                    (rootDir = relPath.substr(0, idx + 1)) &&
                    $.inArray(rootDir, mkdirs) === -1
                  ) {
            ...
            if (mkdirs.length) {
              (function () {
                ...
                fm.uploads.checkExists(checkDirs, target, fm, true).done(function (res, res2) {
                  ...
                  files = $.grep(files, function (file) {
                    return file._remove ? false : true;
                  });
                  paths = $.grep(paths, function (path) {
                    return path === void 0 ? false : true;
                  });
                  if (checkDirs.length) {
                    dfd = $.Deferred();
                    if (res.length) {
              ...
              fm.uploads.checkExists(files, target, fm).done(function (res, res2) {
                if (fm.options.overwriteUploadConfirm && fm.option("uploadOverwrite", target)) {
                  ...
                  files = $.grep(files, function (file) {
                    return !file._remove ? true : false;
                  });
                }
                cnt = files.length;
                if (cnt > 0) {
        ...
        if (dataChecked) {
          ...
          files
            .done(function (result) {
              // result: [files, paths, renames, hashes, mkdirs]
              renames = [];
              cnt = result[0].length;
              if (cnt) {
    ...
    iframe: function (data, fm) {
      var self = fm ? fm : this,
        ...
        iframe = $(
          '<iframe src="' +
            (msie ? "javascript:false;" : "about:blank") +
            '" name="' +
            name +
      ...
      if (files && files.length) {
        ...
        if (fm.options.overwriteUploadConfirm && fm.option("uploadOverwrite", target)) {
          ...
          dfds.push(
            self.uploads.checkExists(names, target, self).done(function (res, res2) {
              hashes = res2;
              if (res === null) {
                data.overwrite = 0;
              } else {
      ...
      filter = function (file, asMap, type) {
        ...
        if (file && file.hash && file.name && file.mime) {
          ...
          if (isRoot || file.mime === "directory") {
            ...
            if (isRoot && !file.i18) {
              (name = "volume_" + file.name), (i18 = self.i18n(false, name));
            ...
            if (i18nFolderName && !file.i18) {
              (name = "folder_" + file.name), (i18 = self.i18n(false, name));
  ...
  notify: function (opts) {
    var type = opts.type,
      ...
      msg = this.i18n(
        typeof opts.msg !== "undefined" ? opts.msg : this.messages["ntf" + type] ? "ntf" + type : "ntfsmth"
      ),
      ndialog = this.ui.notify,
      notify = ndialog.children(".elfinder-notify-" + type + (id ? "." + id : "")),
      button = notify.children("div.elfinder-notify-cancel").children("button"),
      ntpl =
        '<div class="elfinder-notify elfinder-notify-{type}' +
        (id ? " " + id : "") +
        '"><span class="elfinder-dialog-icon elfinder-dialog-icon-{type}"/><span class="elfinder-notify-msg">{msg}</span> <span class="elfinder-notify-cnt"/><div class="elfinder-notify-progressbar"><div class="elfinder-notify-progress"/></div><div class="elfinder-notify-cancel"/></div>',
      delta = opts.cnt,
    ...
    if (cnt > 0) {
      ...
      if (progress != null && (total = notify.data("total")) >= 0 && (prc = notify.data("progress")) >= 0) {
        ...
        notify.find(".elfinder-notify-progress").animate(
          {
  ...
  confirm: function (opts) {
    ...
    return this.dialog(
      '<span class="elfinder-dialog-icon elfinder-dialog-icon-confirm"/>' + this.i18n(opts.text),
      options
    );
  },
  ...
  i18n: function () {
    ...
    return $.grep(input, function (m, i) {
      return $.inArray(i, ignore) === -1 ? true : false;
    }).join("<br>");
  },
  ...
  mimeIsText: function (mime) {
    return this.textMimes[mime.toLowerCase()] ||
      (mime.indexOf("text/") === 0 && mime.substr(5, 3) !== "rtf") ||
      mime.match(/^application\/.+\+xml$/)
      ? true
      : false;
  },
  ...
  reloadContents: function (url) {
    ...
    try {
      ifm = $(
        '<iframe width="1" height="1" scrolling="no" frameborder="no" style="position:absolute; top:-1px; left:-1px" crossorigin="use-credentials">'
      )
        .attr("src", url)
        .one("load", function () {
          var ifm = $(this);
  ...
  makeNetmountOptionOauth: function (protocol, name, host, opt) {
    var noOffline = typeof opt === "boolean" ? opt : null, // for backward compat
      ...
      addFolders = function (fm, bro, folders) {
        ...
        bro.next().remove();
        if (cnt) {
          select = $('<select class="ui-corner-all elfinder-tabstop" style="max-width:200px;">')
            .append(
              $(
                $.map(folders, function (n, i) {
    ...
    return {
      ...
      select: function (fm, ev, d) {
        var f = this.inputs,
          ...
          f0 = $(f.host[0]),
          data = d || null;
        this.vars.mbtn = f.host
          .closest(".ui-dialog")
          .children(".ui-dialog-buttonpane:first")
          .find("button.elfinder-btncnt-0");
        if (
          !f0.data("inrequest") &&
          ...
          if (oline.parent().children().length === 1) {
            ...
            oline.uniqueId().after(
              $("<label/>")
                .attr("for", oline.attr("id"))
                .html(" " + fm.i18n("offlineAccess"))
            );
          }
          f0.data("inrequest", true)
            .empty()
            .addClass("elfinder-spinner")
            .parent()
            .find("span.elfinder-button-icon")
            .remove();
          fm.request({
            data: {
              cmd: "netmount",
              ...
              options: {
                id: fm.id,
                offline: oline.prop("checked") ? 1 : 0,
                pass: f.host[1].value,
              },
            },
            preventDefault: true,
          }).done(function (data) {
            f0.removeClass("elfinder-spinner").html(
              data.body.replace(/\{msg:([^}]+)\}/g, function (whole, s1) {
        ...
        if (data.mode == "makebtn") {
          ...
          f0.parent().append(
            $(
              '<span class="elfinder-button-icon elfinder-button-icon-reload" title="' +
                fm.i18n("reAuth") +
                '">'
            ).on("click", function () {
              f1.val("reauth");
    ...
    cache().done(function () {
      ...
      $.each(targets, function () {
        ...
        if (file && (file.sizeInfo || file.mime !== "directory")) {
          cache.push(
            $.Deferred().resolve(file.sizeInfo ? file.sizeInfo : { size: file.size, dirCnt: 0, fileCnt: 1 })
          );
        } else {
          if (!grps[root]) {
  ...
  debug: function (type, m) {
    ...
    if (d && (d === "all" || d[type])) {
      window.console &&
        window.console.log &&
        window.console.log("elfinder debug: [" + type + "] [" + this.id + "]", m);
    }
