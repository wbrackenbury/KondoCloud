...
$.fn.elfinderdialog = function (opts, fm) {
  ...
  var platformWin = window.navigator.platform.indexOf('Win') != -1,
    ...
    fitSize = function (dialog) {
      ...
      if (syncSize.enabled) {
        ...
        if (
          dialog.data('hasResizable') &&
          (dialog.resizable('option', 'maxWidth') < opts.maxWidth ||
            dialog.resizable('option', 'maxHeight') < opts.maxHeight)
        ) {
    ...
    checkEditing = function () {
      var cldialog = 'elfinder-dialog',
        dialogs = elfNode.children(
          '.' + cldialog + '.' + fm.res('class', 'editing') + ':visible',
        );
  ...
  if (opts.headerBtnOrder && opts.headerBtnOrder === 'auto') {
    opts.headerBtnOrder = platformWin
  ...
  this.filter(':not(.ui-dialog-content)').each(function () {
    var self = $(this).addClass('ui-dialog-content ui-widget-content'),
      ...
      titlebar = $(
        '<div class="ui-dialog-titlebar ui-widget-header ui-corner-top ui-helper-clearfix"><span class="elfinder-dialog-title">' +
          opts.title +
          '</span></div>',
      ),
      ...
      tabstopsInit = function () {
        ...
        if (tabstops.length) {
          ...
          if (!tabstops.filter('.' + cl1stfocus).length) {
            buttonset
              .children(
                '.' + cltabstop + ':' + (platformWin ? 'first' : 'last'),
              )
              .addClass(cl1stfocus);
          }
        }
      ...
      makeHeaderBtn = function () {
        ...
        if (platformWin) {
          titlebar
            .children('.elfinder-titlebar-button')
            .addClass('elfinder-titlebar-button-right');
        }
      },
      headerBtns = {
        close: function () {
          titlebar.prepend(
            $(
              '<span class="ui-widget-header ui-dialog-titlebar-close ui-corner-all elfinder-titlebar-button"><span class="ui-icon ui-icon-closethick"/></span>',
          ...
          if (opts.allowMaximize) {
            dialog.on('resize', function (e, data) {
              ...
              if (data && data.maximize) {
                ...
                if (full) {
                  try {
                    dialog.hasClass('ui-draggable') &&
                      dialog.draggable('disable');
                    dialog.hasClass('ui-resizable') &&
            ...
            titlebar.prepend(
              $(
                '<span class="ui-widget-header ui-corner-all elfinder-titlebar-button elfinder-titlebar-full"><span class="ui-icon ui-icon-plusthick"/></span>',
              ).on('mousedown', function (e) {
                var elm = $(this);
                e.preventDefault();
                e.stopPropagation();
                if (
                  !dialog.hasClass('elfinder-maximized') &&
                  typeof elm.data('style') === 'undefined'
                ) {
                  self.height(self.height());
                  elm.data('style', self.attr('style') || '');
                }
                fm.toggleMaximize(dialog);
                typeof opts.maximize === 'function' &&
                  opts.maximize.call(self[0]);
              }),
            );
          }
        },
        minimize: function () {
          ...
          if (opts.allowMinimize) {
            btn = $(
              '<span class="ui-widget-header ui-corner-all elfinder-titlebar-button elfinder-titlebar-minimize"><span class="ui-icon ui-icon-minusthick"/></span>',
            ).on('mousedown', function (e) {
              var $this = $(this),
                ...
                dum = $('<div/>')
                  .css(dumStyle)
                  .addClass(
                    dialog.get(0).className + ' elfinder-dialog-minimized',
                  ),
                pos = {};
              ...
              if (!dialog.data('minimized')) {
                ...
                mnode
                  ...
                  .children('.ui-dialog-content:first')
                  .empty();
                fm.toHide(dialog.before(mnode));
                mnode
                  .children(
                    '.ui-dialog-content:first,.ui-dialog-buttonpane,.ui-resizable-handle',
                  )
                  .remove();
                mnode
                  .find('.elfinder-titlebar-minimize,.elfinder-titlebar-full')
                  .remove();
                mnode
                ...
                mnode.animate(pos, function () {
                  mnode
                    ...
                    .appendTo(tray);
                  checkEditing();
                  typeof opts.minimize === 'function' &&
                    opts.minimize.call(self[0]);
                });
              } else {
                //restore
                dialog
                  .removeData('minimized')
                  .before(
                    mnode.css(
                      Object.assign({position: 'absolute'}, mnode.offset()),
                    ),
                  );
                fm.toFront(mnode);
                mnode.animate(
            ...
            titlebar
              .on('dblclick', function (e) {
                $(this)
                  .children('.elfinder-titlebar-minimize')
                  .trigger('mousedown');
              })
              .prepend(btn);
      ...
      dialog = $(
        '<div class="ui-front ui-dialog ui-widget ui-widget-content ui-corner-all ui-draggable std42-dialog touch-punch ' +
          cldialog +
          ' ' +
          opts.cssClass +
          '"/>',
        ...
        .on(
          'touchstart touchmove touchend click dblclick mouseup mouseenter mouseleave mouseout mouseover mousemove',
          function (e) {
        ...
        .on('mousedown', function (e) {
          ...
          requestAnimationFrame(function () {
            if (
              dialog.is(':visible') &&
              !dialog.hasClass('elfinder-frontmost')
            ) {
              toFocusNode = $(':focus');
        ...
        .on('open', function () {
          ...
          if (!dialog.hasClass(clnotify)) {
            elfNode
              .children('.' + cldialog + ':visible:not(.' + clnotify + ')')
              .each(function () {
                var d = $(this),
        ...
        .on('totop frontmost', function () {
          ...
          dialog.data(
            'focusOnMouseOver',
            s ? s > 0 : fm.options.uiOptions.dialog.focusOnMouseOver,
          );
          ...
          elfNode
            .children('.' + cldialog + ':not(.' + clmodal + ')')
            .removeClass(clactive);
        ...
        .on('posinit', function () {
          ...
          if (!css && !dialog.data('resizing')) {
            nodeFull =
              elfNode.hasClass('elfinder-fullscreen') ||
              fm.options.enableAlways;
            dialog.css(
              nodeFull
                ? {
                    maxWidth: '100%',
            ...
            css = {
              top:
                outerSize.height >= winSize.height
                  ? minTop
                  : Math.max(
                      minTop,
                      parseInt((elfNode.height() - outerSize.height) / 2 - 42),
                    ),
              left:
                outerSize.width >= winSize.width
                  ? minLeft
                  : Math.max(
                      minLeft,
                      parseInt((elfNode.width() - outerSize.width) / 2),
                    ),
            };
        ...
        .on('resize', function (e, data) {
          var oh = 0,
            ...
            maxH;
          if (
            (data && (data.minimize || data.maxmize)) ||
            dialog.data('minimized')
          ) {
            return;
          }
          e.stopPropagation();
          e.preventDefault();
          dialog
            .children('.ui-widget-header,.ui-dialog-buttonpane')
            .each(function () {
          ...
          if (
            ...
            if (minH.match(/%/)) {
              minH = Math.floor(
                (parseInt(minH) / 100) * dialog.parent().height(),
              );
            } else {
              minH = parseInt(minH);
            }
            if (maxH.match(/%/)) {
              maxH = Math.floor(
                (parseInt(maxH) / 100) * dialog.parent().height(),
              );
            } else {
              maxH = parseInt(maxH);
            }
            h = Math.min(
              syncSize.defaultSize.height,
              Math.max(maxH, minH) - oh - dialog.data('margin-y'),
            );
          } else {
          ...
          minH = self.height();
          minH =
            h < minH ? minH + oh + dialog.data('margin-y') : opts.minHeight;
          dialog.css('min-height', minH);
          dialog.data('hasResizable') &&
            dialog.resizable('option', {minHeight: minH});
        ...
        .on('click', 'select.' + cltabstop, function () {
          var node = $(this);
          node.data('keepFocus')
            ? node.removeData('keepFocus')
            : node.data('keepFocus', true);
        })
        .on('blur', '.' + cltabstop, function () {
          $(this)
            .removeClass(clhover)
            .removeData('keepFocus')
            .parent('label')
            .removeClass(clhover);
          this.id &&
            $(this)
        ...
        .on('mouseenter mouseleave', '.' + cltabstop + ',label', function (e) {
          ...
          if (this.nodeName === 'LABEL') {
            if (
              !$this.children('.' + cltabstop).length &&
              (!(labelfor = $this.attr('for')) ||
                !$('#' + labelfor).hasClass(cltabstop))
            ) {
              return;
            }
          }
        ...
        .on('keydown', '.' + cltabstop, function (e) {
          ...
          if ($this.is(':focus')) {
            ...
            if (e.keyCode === $.ui.keyCode.ENTER) {
              ...
              (e.keyCode === $.ui.keyCode.TAB && e.shiftKey) ||
            ...
            if (
              ...
              (($this.is('textarea') && !(e.ctrlKey || e.metaKey)) ||
                ($this.is('select,span.ui-slider-handle') &&
                  e.keyCode !== $.ui.keyCode.TAB) ||
                ($this.is('input:not(:checkbox,:radio)') &&
                  !(e.ctrlKey || e.metaKey) &&
                  e.keyCode ===
                    $.ui.keyCode[move === 'prev' ? 'LEFT' : 'RIGHT']))
            ) {
              e.stopPropagation();
              return;
            }
            ...
            if (move) {
              e.preventDefault();
              (move === 'prev' ? tabstopPrev : tabstopNext)(this).trigger(
                'focus',
              );
            }
          }
      ...
      posCheck = function () {
        ...
        if (node.hasClass('elfinder-fullscreen')) {
          ...
          dialog.css(
            'top',
            Math.max(Math.min(Math.max(pos.top, 0), node.height() - 100), 0),
          );
          dialog.css(
            'left',
            Math.max(Math.min(Math.max(pos.left, 0), node.width() - 200), 0),
          );
        }
      },
      maxSize,
    ...
    if (syncSize.enabled) {
      delta.width =
        dialog.outerWidth(true) -
        dialog.width() +
        (dialog.outerWidth() - dialog.width()) / 2;
      delta.height =
        dialog.outerHeight(true) -
        dialog.height() +
        (dialog.outerHeight() - dialog.height()) / 2;
    }
    ...
    if (fm.options.dialogContained) {
      ...
      opts.maxWidth = opts.maxWidth
        ? Math.min(maxSize.maxWidth, opts.maxWidth)
        : maxSize.maxWidth;
      opts.maxHeight = opts.maxHeight
        ? Math.min(maxSize.maxHeight, opts.maxHeight)
        : maxSize.maxHeight;
      dialog.css(maxSize);
    }
