  ...
  this.not(".elfinder-cwd").each(function () {
    ...
    var mobile = fm.UA.Mobile,
      ...
      makeTemplateRow = function () {
        return `<tr id="{id}" class="${clFile} {permsclass} {dirclass}" title="{tooltip}"{css}>
            <td class="elfinder-col-name">
              <div class="elfinder-cwd-file-wrapper">
                <span class="elfinder-cwd-icon {mime} {filetype}"{style}/>
                {marker}
                <span class="elfinder-cwd-filename">{name}</span>
              </div>
              ${selectCheckbox}
            </td>
            ${customColsBuild()}
      ...
      selectCheckbox = $.map(options.showSelectCheckboxUA, function (t) {
        return fm.UA[t] || t.match(/^all$/i) ? true : null;
      }).length
        ? `<div class="elfinder-cwd-select"><input type="checkbox" class="${clSelChk}"></div>`
        : "",
      colResizing = false,
      colWidth = null,
      /**
       ...
       **/
      templates = {
        icon: `<div id="{id}" class="${clFile} {permsclass} {dirclass} ui-corner-all" title="{tooltip}">
            <div class="elfinder-cwd-file-wrapper ui-corner-all">
              <div class="elfinder-cwd-icon {mime} ui-corner-all {filetype}" unselectable="on"{style}/>
              {marker}
            </div>
            <div class="elfinder-cwd-filename" title="{nametitle}">{name}</div>
            ${selectCheckbox}
          </div>`,
        row: "",
      },
      ...
      replacement = {
        ...
        filetype: function (f) {
          ...
          const preciseType = words.length ? words[words.length - 1] : "";
          // if theres no dots then it's probably a folder
          if (!f.name.includes(".")) {
            return "";
          }
          const types = {
            // prettier-ignore
            text: ["txt","doc","docx","rtf","dotx","dot","odt","pages","tex","pdf","ps","eps","prn","pages"],
            image: ["jpg", "jpeg", "png", "tiff", "tif", "gif", "bmp", "heic", "tga", "dae"],
            spreadsheet: ["tsv", "csv", "xls", "xlsx", "xltx", "xlt", "ods", "xlsb", "xlsm", "xltm", "json"],
            // prettier-ignore
            media: ["mtp","itlp","nef","asf","cbr","flv","djvu","mod","mpg","cur","psd","3gp","pak","flac","heic","thm","grf","mkv","fh10","wma","pdn","map","hip","ts","ogg","itl","aep","avi","mov","mid","m4a","cdr","ai","xpl","rgssad","srt","vdf","ico","fh11","wav","aiff","feq","3gpp","gc","ithmb","itdb","m4v","m3u","wad","sfl","sfk","lvl","mp4","aif","mp3","ipa","jps","indd","vfs","itc2","kmz","cr2","dss",],
            presentation: ["ppt", "pptx", "odp"],
          };
          // for each possible type check if any of the stuff between the parens is exactly that type
          const fileType = Object.keys(types).find((type) =>
            types[type].some((possibleExtension) => words.some((word) => word === possibleExtension))
          );
          return `${fileType ? `file-type-icon-${fileType}` : ""} precise-file-type-icon-${preciseType}`;
        },
        size: function (f) {
        ...
        marker: function (f) {
          return (
            (f.alias || f.mime == "symlink-broken" ? symlinkTpl : "") +
            (!f.read || !f.write ? permsTpl : "") +
            (f.locked ? lockTpl : "")
          );
        },
        tooltip: function (f) {
      ...
      addBadgeStyle = function (mime, name) {
        ...
        if (mime && !addedBadges[mime]) {
          if (typeof addBadgeStyleSheet === "undefined") {
            ...
            addBadgeStyleSheet =
              $(`<style id="addBadgeStyle${fm.namespace}"/>`)
          ...
          if (addBadgeStyleSheet) {
            ...
            type = mime.split("/");
            ext = fm.escape(
              fm.mimeTypes[mime] || (name.replace(/.bac?k$/i, "").match(/\.([^.]+)$/) || ["", ""])[1]
            );
            if (ext) {
      ...
      /**
       * jQuery node that will be selected next
       *
      ...
      select = function (keyCode, append) {
        ...
        if (sel.length) {
          ...
          if (selectedNext.length) {
            ...
            n = cwd.find(
              "[id]:not(." + clDisabled + "):not(.elfinder-cwd-parent):" + (prev ? "last" : "first")
            );
          }
        }
        ...
        fm.lazy(
          function () {
            ...
            if (fm.maxTargets && (incHashes || cwdHashes).length > fm.maxTargets) {
              ...
              fm.toast({
                mode: "warning",
                msg: fm.i18n(["errMaxTargets", fm.maxTargets]),
              });
            } else {
      ...
      trigger = function () {
        ...
        if (
          oldSchoolItem &&
          (selected.length > 1 || selected[0] !== fm.cwdId2Hash(oldSchoolItem.attr("id"))) &&
          oldSchoolItem.hasClass(clSelected)
        ) {
          oldSchoolItem.trigger(evtUnselect);
        }
        allSelected =
          selected.length &&
          selected.length === (incHashes || cwdHashes).length &&
          (!fm.maxTargets || selected.length <= fm.maxTargets);
        if (selectCheckbox) {
      ...
      wrapperRepaint = function (init, recnt) {
        ...
        var firstNode = (list ? cwd.find("tbody:first") : cwd).children(
          "[id]" + (options.oldSchool ? ":not(.elfinder-cwd-parent)" : "") + ":first"
        );
        if (!firstNode.length) {
          return;
        }
        var selectable = cwd.data("selectable"),
          rec = (function () {
            var wos = wrapper.offset(),
              ...
              x = firstNode.width() / 2,
              l = Math.min(
                wos.left - w.scrollLeft() + (fm.direction === "ltr" ? x : ww - x),
                wos.left + ww - 10
              ),
          ...
          multi = 5,
          cnt = Math.ceil(
            (bufferExt.hpi ? Math.ceil((wz.data("rectangle").height / bufferExt.hpi) * 1.5) : showFiles) /
              multi
          ),
          chk = function () {
            var id, hash, file, i;
            ...
            if (cwd.data("selectable")) {
              ...
              if (idsArr.length) {
                ...
                cwd
                  .selectable("enable")
                  .selectable("option", { filter: selectableOption.filter })
                  .selectable("refresh");
              }
            }
        ...
        if (tgt.length) {
          if (tgt.attr("id")) {
            if (init) {
              ...
              bufferExt.repaintJob &&
                bufferExt.repaintJob.state() === "pending" &&
                bufferExt.repaintJob.reject();
              arr = new Array(cnt);
              bufferExt.repaintJob = fm
                .asyncJob(function () {
                  chk();
                  if (!tgt.length) {
                    done();
        ...
        var phash = fm.cwd().phash,
          ...
          set = function (pdir) {
            if (pdir) {
              ...
              (list ? oldSchoolItem.children("td:first") : oldSchoolItem)
                .children(".elfinder-cwd-select")
                .remove();
        ...
        if (!bufferExt.rendering && buffer.length) {
          ...
          if (
            (chk =
              wrapper.height() +
              wrapper.scrollTop() +
              fm.options.showThreshold +
              bufferExt.row -
              bufferExt.renderd * bufferExt.hpi) > 0
          ) {
            bufferExt.rendering = true;
            fm.lazy(function () {
              go(chk);
      ...
      fixTableHeader = function (optsArg) {
        ...
        var setPos = function () {
            ...
            val =
              (fm.direction === "ltr"
                ? wrapper.scrollLeft()
                : table.outerWidth(true) - wrapper.width() - wrapper.scrollLeft()) * -1;
            if (base.css(pos) !== val) {
              base.css(pos, val);
            }
          },
      ...
      droppable = Object.assign({}, fm.droppable, {
        ...
        out: function (e, ui) {
          ...
          e.stopPropagation();
          helper
            .removeClass("elfinder-drag-helper-move elfinder-drag-helper-plus")
            .data("dropover", Math.max(helper.data("dropover") - 1, 0));
            ...
            if (node.length) {
              if (tmb != "1") {
                ...
                if (reload) {
                  node
                    .find(".elfinder-cwd-icon")
                    .addClass(tmbObj.className)
                    .css("background-image", "url('" + tmbObj.url + "')");
                } else {
                  attach(node, tmbObj);
                }
      ...
      add = function (files, mode) {
        ...
        if (l > showFiles) {
          ...
          while (l--) {
            ...
            if (fm.cwdHash2Elm(hash).length) {
              if (
                (file.tmb && (file.tmb != 1 || file.size > 0)) ||
                (stmb && file.mime.indexOf("image/") === 0)
              ) {
      ...
      customColsNameBuild = function () {
        ...
        for (var i = 0; i < customCols.length; i++) {
          ...
          customColsName +=
            '<td class="elfinder-cwd-view-th-' + customCols[i] + ' sortable-item">' + name + "</td>";
        }
          ...
          if (e.type === "touchmove") {
            if (
              ...
              Math.abs(wrapper.data("touching").x - e.originalEvent.touches[0].pageX) +
                Math.abs(wrapper.data("touching").y - e.originalEvent.touches[0].pageY) >
                4
            ) {
      ...
      content = function () {
        fm.lazy(function () {
          ...
          wz.append(selectAllCheckbox).removeClass(
            "elfinder-cwd-wrapper-empty elfinder-search-result elfinder-incsearch-result elfinder-letsearch-result"
          );
          if (fm.searchStatus.state > 1 || fm.searchStatus.ininc) {
            wz.addClass(
              "elfinder-search-result" +
                (fm.searchStatus.ininc
                  ? " elfinder-" + (query.substr(0, 1) === "/" ? "let" : "inc") + "search-result"
                  : "")
            );
          }
          ...
          wrapper[list ? "addClass" : "removeClass"]("elfinder-cwd-wrapper-list")._padding =
          ...
          if (list) {
            ...
            thtr = $(
              '<tr class="ui-state-default"><td class="elfinder-cwd-view-th-name">' +
                fm.getColumnName("name") +
                "</td>" +
                customColsNameBuild() +
                "</tr>"
            );
            ...
            thtr
              ...
              .resizable({
                ...
                resize: function (e, ui) {
                  ui.element
                    .data("resizeTarget")
                    .width(ui.element.data("targetWidth") - (ui.originalSize.width - ui.size.width));
                },
                stop: function (e, ui) {
                  colResizing = false;
                  fixTableHeader({ fitWidth: true });
          ...
          // set droppable
          if (!fm.cwd().write) {
            wrapper.removeClass("native-droppable").droppable("disable").removeClass("ui-state-disabled"); // for old jQueryUI see https://bugs.jqueryui.com/ticket/5974
          } else {
      ...
      cwd = $(this)
        ...
        // fix selectable bugs and add shift+click support
        .on("click." + fm.namespace, fileSelector, function (e) {
          ...
          if (
            selectCheckbox &&
            (tgt.is("input:checkbox." + clSelChk) || tgt.hasClass("elfinder-cwd-select"))
          ) {
            e.stopPropagation();
            e.preventDefault();
        ...
        .on("touchstart." + fm.namespace, fileSelector, function (e) {
          ...
          if (
            ...
            nodeName === "TEXTAREA" ||
            tgt.hasClass("elfinder-cwd-nonselect")
          ) {
            e.stopPropagation();
            return;
          }
        ...
        .on("mouseenter." + fm.namespace, fileSelector, function (e) {
          ...
          if (
            !mobile &&
            !$this.data("dragRegisted") &&
            !$this.hasClass(clTmp) &&
            !$this.hasClass(clDraggable) &&
            !$this.hasClass(clDisabled)
          ) {
            $this.data("dragRegisted", true);
            if (
              !fm.isCommandEnabled(
                "copy",
                fm.searchStatus.state > 1 || $this.hasClass("isroot")
                  ? fm.cwdId2Hash($this.attr("id"))
                  : void 0
              )
            ) {
              return;
              ...
              .on("dragstart", function (e) {
                ...
                if (dt && !fm.UA.IE) {
                  ...
                  if (geturl.length) {
                    ...
                    if (dt.setDragImage) {
                      helper = $('<div class="elfinder-drag-helper html5-native"></div>')
                        .append(icon(fm.file(files[0])))
                        .appendTo($(document.body));
                      if ((l = files.length) > 1) {
                        helper.append(
                          icon(fm.file(files[l - 1])) + '<span class="elfinder-drag-num">' + l + "</span>"
                        );
                      }
        ...
        .on(evtDisable, fileSelector, function () {
          var $this = $(this)
              .removeClass(clHover + " " + clSelected)
              .addClass(clDisabled),
            child = $this.children(),
        ...
        .on("mouseenter." + fm.namespace + " mouseleave." + fm.namespace, fileSelector, function (e) {
          ...
          fm.trigger("hover", {
            hash: fm.cwdId2Hash($(this).attr("id")),
            type: e.type,
          });
        ...
        .on(
          "mouseenter." + fm.namespace + " mouseleave." + fm.namespace,
          ".elfinder-cwd-file-wrapper,.elfinder-cwd-filename",
          function (e) {
            var enter = e.type === "mouseenter";
            if (enter && scrolling) {
              return;
            ...
            $(this)
              .closest(fileSelector)
              .children(".elfinder-cwd-file-wrapper,.elfinder-cwd-filename")
              .toggleClass(clActive, e.type == "mouseenter");
          }
        )
      ...
      wrapper = $('<div class="elfinder-cwd-wrapper"/>')
        ...
        .on("scroll." + fm.namespace, function () {
          ...
          if (
            bufferExt.scrtm &&
            Math.abs(
              (bufferExt.scrolltop || 0) - (bufferExt.scrolltop = this.scrollTop || $(this).scrollTop())
            ) < 5
          ) {
            bufferExt.scrtm = 0;
            wrapper.trigger(scrollEvent);
      ...
      selectAllCheckbox = selectCheckbox
        ? $('<div class="elfinder-cwd-selectall"><input type="checkbox"/></div>')
    ...
    fm.one("init", function () {
      ...
      if (document.head) {
        ...
        sheet.insertRule(
          '.elfinder-cwd-wrapper-empty .native-droppable .elfinder-cwd:not(.elfinder-table-header-sticky):after{ content:"' +
            fm.i18n("emptyFolder" + (mobile ? "LTap" : "Drop")) +
            '" }',
          i++
        );
        sheet.insertRule(
        ...
        sheet.insertRule(
          '.elfinder-cwd-wrapper-empty.elfinder-search-result.elfinder-incsearch-result .elfinder-cwd:not(.elfinder-table-header-sticky):after{ content:"' +
            fm.i18n("emptyIncSearch") +
            '" }',
          i++
        );
        sheet.insertRule(
            ...
            if (q) {
              ...
              if (type === "SearchName") {
                incHashes = $.grep(cwdHashes, function (hash) {
                  ...
                  return file && (file.name.match(regex) || (file.i18 && file.i18.match(regex)))
                    ? true
                    : false;
                });
              } else {
      ...
      .add(function (e) {
        var regex = query
            ? new RegExp(query.replace(/([\\*\;\.\?\[\]\{\}\(\)\^\$\-\|])/g, "\\$1"), "i")
            : null,
            ...
            if (!res && inSearch) {
              ...
              if (!res && fm.searchStatus.mixed) {
                res = $.grep(fm.searchStatus.mixed, function (vid) {
                  return f.hash.indexOf(vid) === 0 ? true : false;
                }).length
                  ? true
                  : false;
              }
      ...
      .change(function (e) {
        ...
        if (query) {
          ...
          $.each(
            $.grep(e.data.changed || [], function (f) {
              return f.phash == phash ? true : false;
            }),
            function (i, file) {
              if (fm.cwdHash2Elm(file.hash).length) {
                remove([file.hash]);
      ...
      .remove(function (e) {
        ...
        if (
          buffer.length < 1 &&
          place.children(fileSelector + (options.oldSchool ? ":not(.elfinder-cwd-parent)" : "")).length < 1
        ) {
        ...
        callback: function (e) {
          if (bufferExt.itemH) {
            wrapper.scrollTop(
              Math.round(
                wrapper.scrollTop() -
                  Math.floor((wrapper.height() + (list ? bufferExt.itemH * -1 : 16)) / bufferExt.itemH) *
                    bufferExt.itemH
              )
            );
          }
