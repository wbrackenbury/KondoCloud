...
$.fn.elfinderpath = function (fm, options) {
  ...
  return this.each(function () {
    var query = '',
      ...
      prefix =
        'path' +
        (elFinder.prototype.uniqueid ? elFinder.prototype.uniqueid : '') +
        '-',
      wzbase = $(
        '<div class="ui-widget-header ui-helper-clearfix elfinder-workzone-path"/>',
      ),
      path = $(this)
      ...
      roots = $('<div class="elfinder-path-roots"/>')
        ...
        .append(
          '<span class="elfinder-button-icon elfinder-button-icon-menu" />',
        )
        .appendTo(wzbase),
      render = function (cwd) {
        var dirs = [],
        ...
        $.each(fm.parents(cwd), function (i, hash) {
          ...
          dirs.push(
            '<span id="' +
              ...
              c +
              '" title="' +
              names.join(fm.option('separator')) +
              '">' +
              name +
              '</span>',
          );
        ...
        return dirs.join(
          '<span class="elfinder-path-other">' +
            fm.option('separator') +
            '</span>',
        );
      },
      toWorkzone = function () {
        ...
        prev =
          fm.direction === 'ltr'
            ? $('#' + prefix + fm.cwd().hash).prevAll(
                'span.elfinder-path-dir:first',
              )
            : $();
