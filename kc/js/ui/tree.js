...
$.fn.elfindertree = function (fm, opts) {
  ...
  this.not('.' + treeclass).each(function () {
    var c = 'class',
      ...
      execSubdirs = function () {
        var cnt = opts.subdirsMaxConn - subdirsPending,
          atOnce = fm.maxTargets
            ? Math.min(fm.maxTargets, opts.subdirsAtOnce)
            : opts.subdirsAtOnce,
          i,
          ids;
      ...
      droppableopts = $.extend(true, {}, fm.droppable, {
        ...
        over: function (e, ui) {
          ...
          if (
            ui.helper.data('namespace') !== fm.namespace ||
            !fm.insideWorkzone(e.pageX, e.pageY)
          ) {
            dst.removeClass(cl);
            helper.removeClass(
              'elfinder-drag-helper-move elfinder-drag-helper-plus',
            );
            return;
          }
          ...
          if (dst.is('.' + collapsed + ':not(.' + expanded + ')')) {
            dst.data(
              'expandTimer',
              setTimeout(function () {
                dst.is('.' + collapsed + '.' + hover) &&
                  dst.children('.' + arrow).trigger('click');
              }, 500),
            );
          }
          ...
          $.each(ui.helper.data('files'), function (i, h) {
            if (
              h === hash ||
              (fm.file(h).phash === hash &&
                !ui.helper.hasClass('elfinder-drag-helper-plus'))
            ) {
              dst.removeClass(cl);
      ...
      replace = {
        ...
        title: function (dir) {
          return opts.attrTitle
            ? ' title="' +
                fm.escape(fm.path(dir.hash, true) || dir.i18 || dir.name) +
                '"'
            : '';
        },
        root: function (dir) {
      ...
      itemhtml = function (dir) {
        return tpl.replace(/(?:\{([a-z]+)\})/gi, function (m, key) {
      ...
      filter = function (files, checkExists) {
        return $.map(files || [], function (f) {
          return f.mime === 'directory' &&
            (!checkExists || fm.navHash2Elm(f.hash).length)
            ? f
            : null;
        });
      },
      ...
      updateTree = function (dirs) {
        var length = dirs.length,
          ...
          append = function (parent, dirs, start, direction) {
            var hashes = {},
              curStart = 0,
              max = fm.newAPI
                ? Math.min(10000, Math.max(10, opts.subTreeMax))
                : 10000,
              setHashes = function () {
                hashes = {};
              ...
              render = function (start, direction) {
                ...
                if (prev > -1) {
                  prevBtn = $(
                    '<button class="elfinder-navbar-pager elfinder-navbar-pager-prev"/>',
                  )
                ...
                if (next) {
                  nextBtn = $(
                    '<button class="elfinder-navbar-pager elfinder-navbar-pager-next"/>',
                  )
                ...
                parent
                  .empty()
                  [parts ? 'addClass' : 'removeClass'](
                    'elfinder-navbar-hasmore',
                  )
                  .append(prevBtn, html.join(''), nextBtn);
                ...
                if (direction) {
                  autoScroll(
                    fm.navHash2Id(
                      parts[direction === 'up' ? parts.length - 1 : 0].hash,
                    ),
                  );
                }
                !mobile &&
              ...
              detach = function () {
                $.each(
                  parent.children('.elfinder-navbar-wrapper'),
                  function (i, elm) {
                    var n = $(elm),
        ...
        while (i--) {
          ...
          if ((parent = findSubtree(dir.phash)).length) {
            if (
              dir.phash &&
              ((init = !parent.children().length) ||
                parent.hasClass('elfinder-navbar-hasmore') ||
                (sibling = findSibling(parent, dir)).length)
            ) {
              if (init) {
                if (!atonce[dir.phash]) {
                  atonce[dir.phash] = [];
        ...
        !mobile &&
          tgts.length &&
          fm.lazy(function () {
            updateDroppable(tgts.children('span'));
          });
      },
      /**
      ...
      compare = function (dir1, dir2) {
        if (!fm.sortAlsoTreeview) {
          ...
          res = asc
            ? rules[fm.sortType](dir1, dir2)
            : rules[fm.sortType](dir2, dir1);
      ...
      autoScroll = function (target) {
        ...
        autoScrTm = setTimeout(function () {
          current = $(
            document.getElementById(target || fm.navHash2Id(fm.cwd().hash)),
          );
          if (current.length) {
            // expand parents directory
            (openCwd ? current : current.parent())
              .parents('.elfinder-navbar-wrapper')
              .children('.' + loaded)
              .addClass(expanded)
              .next('.' + subtree)
              .show();
        ...
        if (!node.length) {
          while (cur && cur.phash) {
            if (hasMoreDirs[cur.phash] && !fm.navHash2Elm(cur.hash).length) {
              hasMoreDirs[cur.phash].trigger('update.' + fm.namespace, {
                select: cur.hash,
              });
            }
        ...
        var cwd = fm.cwd(),
          ...
          loadParents = function (dir) {
            ...
            if (reqs.length) {
              ...
              baseNode = $('#' + baseId);
              spinner = $(fm.res('tpl', 'navspinner')).insertBefore(
                baseNode.children('.' + arrow),
              );
      ...
      updateDroppable = function (target, node) {
        ...
        if (!target) {
          if (
            !node ||
            node.closest('div.' + wrapperRoot).hasClass(uploadable)
          ) {
            (node || tree.find('div.' + uploadable))
              .find(selNavdir + ':not(.elfinder-ro,.elfinder-na)')
              .addClass('native-droppable');
          }
          if (!node || node.closest('div.' + wrapperRoot).hasClass(pastable)) {
            target = (node || tree.find('div.' + pastable)).find(
              selNavdir + ':not(.' + droppable + ')',
            );
          } else {
            target = $();
          }
      ...
      tree = $(this)
        ...
        .on('mouseenter mouseleave', selNavdir, function (e) {
          ...
          if (!link.hasClass(dropover + ' ' + disabled)) {
            if (
              !mobile &&
              enter &&
              !link.data('dragRegisted') &&
              !link.hasClass(
                root + ' ' + draggable + ' elfinder-na elfinder-wo',
              )
            ) {
        ...
        .on('click', selNavdir + '.' + collapsed + ' .' + arrow, function (e) {
          ...
          if (link.hasClass(loaded)) {
            ...
            fm.lazy(function () {
              cnt = link.hasClass(expanded)
                ? stree.children().length +
                  stree
                    .find('div.elfinder-navbar-subtree[style*=block]')
                    .children().length
                : stree.find('div:visible').length;
              if (cnt > slideTH) {
                stree.toggle();
                fm.draggingUiHelper &&
                  fm.draggingUiHelper.data('refreshPositions', 1);
                checkSubdirs();
              } else {
                stree
                  .stop(true, true)
                  [link.hasClass(expanded) ? 'slideDown' : 'slideUp'](
                    opts.durations.slideUpDown,
                    function () {
                      fm.draggingUiHelper &&
                        fm.draggingUiHelper.data('refreshPositions', 1);
                      checkSubdirs();
              ...
              .done(function (data) {
                ...
                if (stree.children().length) {
                  ...
                  if (stree.children().length > slideTH) {
                    ...
                    stree
                      .stop(true, true)
                      .slideDown(opts.durations.slideUpDown, function () {
                        fm.draggingUiHelper &&
                          fm.draggingUiHelper.data('refreshPositions', 1);
                        checkSubdirs();
                      });
    ...
    fm.open(function (e) {
      ...
      if (fm.UA.iOS) {
        navbar
          .removeClass('overflow-scrolling-touch')
          .addClass('overflow-scrolling-touch');
      }
        ...
        while (l--) {
          ...
          if ((node = fm.navHash2Elm(dir.hash)).length) {
            ...
            if (phash) {
              ...
              if (
                reqParent[0] !== realParent[0] ||
                realSibling.get(0) !== reqSibling.get(0)
              ) {
                reqSibling.length
                  ? reqSibling.before(parent)
                  : reqParent.append(parent);
              }
            }
          ...
          if ((node = fm.navHash2Elm(dirs[l])).length) {
            ...
            if (!stree.children().length) {
              stree
                .hide()
                .prev(selNavdir)
                .removeClass(collapsed + ' ' + expanded + ' ' + loaded);
            }
          }
        }

        removed &&
          fm.getUI('navbar').children('.ui-resizable-handle').trigger('resize');
