  ...
  return this.each(function () {
    var dirs = {},
      ...
      create = function (dir, hash) {
        return $(
          tpl
            .replace(/\{id\}/, hash2id(dir ? dir.hash : hash))
            .replace(/\{name\}/, fm.escape(dir ? dir.i18 || dir.name : hash))
            .replace(
              /\{cssclass\}/,
              dir
                ? fm.perms2class(dir) +
                    (dir.notfound ? ' elfinder-na' : '') +
                    (dir.csscls ? ' ' + dir.csscls : '')
                : '',
            )
            .replace(
              /\{permissions\}/,
              dir && (!dir.read || !dir.write || dir.notfound) ? ptpl : '',
            )
            .replace(
              /\{title\}/,
              dir
        ...
        $.each(dirs, function (h, f) {
          ...
          if (subtree.children().length) {
            $.each(subtree.children(), function () {
              var current = $(this);
              if (
                (dir.i18 || dir.name).localeCompare(
                  current.children('.' + navdir).text(),
                ) < 0
              ) {
      ...
      // sort button
      sortBtn = $(
        '<span class="elfinder-button-icon elfinder-button-icon-sort elfinder-places-root-icon" title="' +
          fm.i18n('cmdsort') +
          '"/>',
      )
        .hide()
       ...
       **/
      places = $(this)
        .addClass(fm.res(c, 'tree') + ' elfinder-places ui-corner-all')
        .hide()
        .append(wrapper)
        ...
        .droppable({
          ...
          over: function (e, ui) {
            var helper = ui.helper,
              dir = $.grep(helper.data('files'), function (h) {
                return fm.file(h).mime === 'directory' && !dirs[h]
                  ? true
                  : false;
              });
            e.stopPropagation();
            ...
            if (fm.insideWorkzone(e.pageX, e.pageY)) {
              if (dir.length > 0) {
                ...
                fm.trigger('unlockfiles', {
                  files: helper.data('files'),
                  helper: helper,
                });
              } else {
    ...
    if ($.fn.sortable) {
      subtree.addClass('touch-punch').sortable({
        ...
        helper: function (e) {
          ...
          return $(
            '<div class="ui-widget elfinder-place-drag elfinder-' +
              fm.direction +
              '"/>',
          ).append(
            $('<div class="elfinder-navbar"/>').show().append(dir.clone()),
          );
        },
        stop: function (e, ui) {
              ...
              .done(function (data) {
                ...
                $.each(dirs, function (h, f) {
                  if (f.notfound === Boolean(exists[h])) {
                    if (
                      (f.phash === cwd && ev.type !== 'netmount') ||
                      (exists[h] && exists[h].mime !== 'directory')
                    ) {
                      if (remove(h)) {
                        updated = true;
                      }
                      ...
                      if (
                        update(exists[h] || Object.assign({notfound: true}, f))
                      ) {
                        updated = true;
                      }
                    }
