...
$.fn.elfinderstat = function (fm) {
  ...
  return this.each(function () {
    ...
    var sel = $('<div class="elfinder-stat-selected"/>').on("click", "a", function (e) {
      var hash = $(this).data("hash");
      e.preventDefault();
      fm.exec("opendir", [hash]);
    });
    var titleitems = fm.i18n("items");
    var titlesel = fm.i18n("selected");
    var titlesize = fm.i18n("size");
    var setstat = function (files) {
        var c = 0,
          s = 0,
          cwd = fm.cwd(),
          calc = true,
        ...
        size
          .html(
            `<div class="stat-content">
              ${titleitems}: 
              <span class="elfinder-stat-incsearch"></span> 
              <b>${c}</b>,&nbsp;
              <span 
              class="elfinder-stat-size${hasSize ? " elfinder-stat-size-recursive" : ""}">
                    ${fm.i18n(hasSize ? "sum" : "size")}: 
                    <b> ${fm.formatSize(s)}</b>
                  </span>
            </div>`
          )
          .attr("title", size.text().trim().replace(/\s*/g, ""));
        fm.trigger("uistatchange");
      },
      setIncsearchStat = function (data) {
        size.find("span.elfinder-stat-incsearch").html(data ? data.hashes.length + " / " : "");
        size.attr("title", size.text().trim().replace(/\s*/g, ""));
        fm.trigger("uistatchange");
      },
      setSelect = function (files) {
        ...
        if (files.length === 1) {
          ...
          if (fm.searchStatus.state === 2) {
            ...
            dirs.push(
              `<a href="#elf_${file.phash}" data-hash="${file.hash}" title="${path}">Current file: ${path}</a>`
            );
          }
          dirs.push(fm.escape(file.i18 || file.name));
          sel.html(
            `<div class="elfinder-stat-selected-content">${
              dirs.join("/") + (s > 0 ? ", " + fm.formatSize(s) : "")
            }</div>`
          );
          ...
          sel.html(
            `<div class="elfinder-stat-selected-content">${
              c ? `${titlesel}: ${c}, ${titlesize}: ${fm.formatSize(s)}` : "&nbsp;"
            }</div>`
          );
        } else {
          sel.html("");
      ...
      .change(function (e) {
        ...
        $.each(files, function () {
          if (this.hash === cwdHash) {
            if (this.size) {
              size
                .children(".elfinder-stat-size")
                .addClass("elfinder-stat-size-recursive")
                .html(fm.i18n("sum") + ": " + fm.formatSize(this.size));
