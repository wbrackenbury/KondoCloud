...
$.fn.elfinderbutton = function (cmd) {
  ...
  return this.each(function () {
      ...
      button = $(this)
        ...
        .on('mouseenter mouseleave', function (e) {
          !button.hasClass(disabled) &&
            button[e.type == 'mouseleave' ? 'removeClass' : 'addClass'](hover);
        })
        .on('click', function (e) {
    ...
    if (Array.isArray(cmd.variants)) {
      ...
      menu = $(
        '<div class="ui-front ui-widget ui-widget-content elfinder-button-menu elfinder-button-' +
          prvCname +
          '-menu ui-corner-all"/>',
      )
        .hide()
        .appendTo(fm.getUI())
        .on('mouseenter mouseleave', '.' + item, function () {
        ...
        $.each(cmd.variants, function (i, variant) {
          menu.append(
            $('<div class="' + item + '">' + variant[1] + '</div>')
              .data('value', variant[0])
              .addClass(variant[0] == cmd.value ? selected : ''),
          );
        });
      });
    }
          ...
          if (cmd.syncTitleOnChange) {
            ...
            if (prvCname !== cName) {
              button
                ...
                .removeClass('elfinder-button-icon-' + prvCname)
                .addClass('elfinder-button-icon-' + cName);
              if (menu) {
                menu
                  .removeClass('elfinder-button-' + prvCname + '-menu')
                  .addClass('elfinder-button-' + cName + '-menu');
              }
              prvCname = cName;
            }
            text.html(cmd.title);
