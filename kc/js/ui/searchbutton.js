...
$.fn.elfindersearchbutton = function (cmd) {
  ...
  return this.each(function () {
    var result = false,
      ...
      search = function () {
        ...
        var val = $.trim(input.val()),
          owner = $('#ownerdropdown').html(),
          type = $('#typedropdown').html(),
          date = $('#datedropdown').html(),
          valusage;
        if (owner === 'Anyone') {
          owner = null;
        }
        ...
        //valusage = $('#qbuttons').children('input:checked').val();
        valusage = $.trim(
          $(
            'label[for=' +
              $('#qbuttons').children('input:checked').attr('id') +
              ']',
          ).text(),
        );
        if (val) {
          input.trigger('focus');
          cmd
            .exec(val, {}, null, null, owner, type, date, valusage)
            .done(function () {
              result = true;
            })
            .fail(function () {
      ...
      input = $('<input type="text" size="42"/>')
        .on('focus', function () {
          ...
          fm.trigger('uiresize');
          opts &&
            opts.css(getMenuOffset()).slideDown(function () {
      ...
      searchdropdown = function (content) {
        var barid;
        if (content === 'owner') {
          barid = 'ownerdropdown';
        ...
        var drop = $("<div id='" + barid + "' class='search-dropdown'></div>");
        drop.css({'flex-direction': 'row', 'justify-content': 'space-between'});
        ...
        drop.data('isopen', false);
        drop.on('click', function (e) {
          if (drop.data('isopen')) {
            drop.data('isopen', false);
            $('#dropcontent').remove();
          } else {
            drop.data('isopen', true);
            var dropcont = $(
              "<div id='dropcontent' class='search-drop-content'></div>",
            );
            dropcont.width(drop.width());
            dropcont.css({
              top: drop.offset().top + drop.height(),
            ...
            var labels;
            if (content === 'owner') {
              labels = ['Anyone', 'Owned by me', 'Not owned by me'];
            } else if (content === 'type') {
              labels = ['All', 'Text', 'Media', 'Other'];
            } else if (content === 'date') {
              labels = [
                'Any time',
                'Last 7 days',
            ...
            for (var i = 0; i < labels.length; i++) {
              if (i != 0) {
                dropcont.append(
                  $("<div class='search-drop-item-border'></div>"),
                );
              }

              //Maybe add a unicode checkmark?

              dropcont.append(
                $("<div class='search-drop-item'>" + labels[i] + '</div>').on(
                  'click',
                  function (e) {
                    //e.stopPropagation();
                    if ($('#' + barid + '').html() !== $(this).html()) {
                      $('#' + barid + '').html($(this).html());
                      drop.data('isopen', false);
                      $('#dropcontent').remove();
                    }
                  },
            ...
            dropcont.appendTo(fm.getUI());
          }
        });

        return drop;
      },
      searchmenurow = function (title, content) {
        var row = $("<div class='search-row-cont'></div>").css({
            display: 'flex',
            'flex-direction': 'row',
            margin: '0px',
          }),
          leftheader = $("<div class='search-row-title'>" + title + '</div>'),
          contbox = $("<div class='search-row-meat'></div>").append(content),
          opt;

        return row.append(leftheader).append(contbox);
      },
      createsearchmenu = function () {
        return $(
          ...
          .append(
            searchmenurow('Type', searchdropdown('type')),
            searchmenurow('Owner', searchdropdown('owner')),
            searchmenurow('Date Modified', searchdropdown('date')),
            $('<div id="qbuttons" class="buttonset"/>')
              .css('flex-direction', 'row')
              .append(
                $(
                  '<input id="filenameradio" name="serchfrom" type="radio" checked="checked"/><label for="filenameradio">Filename</label>',
                ),
                $(
                  '<input id="containswordsradio" name="serchfrom" type="radio"/><label for="containswordsradio">Contains words</label>',
                ),
                $(
                  '<input id="sharedwithradio" name="serchfrom" type="radio"/><label for="sharedwithradio">Shared with</label>',
                ),
              ),
          )
          .hide()
            ...
            input.data(
              ...
              setTimeout(function () {
                ...
                if (val.length === 0 || val.length >= isopts.minlen) {
                  incVal !== val &&
                    fm.trigger('incsearchstart', {
                      query: val,
                      type: typeSet
                        ? typeSet.children('input:checked').val()
                        : 'searchName',
                    });
                  incVal = val;
                  if (
                    val === '' &&
                    fm.searchStatus.state > 1 &&
                    fm.searchStatus.query
                  ) {
      ...
      if (parent.length) {
        ...
        if (fm.UA.ltIE7) {
          var icon = button.children(
            fm.direction == 'ltr' ? '.ui-icon-close' : '.ui-icon-search',
          );
          icon.css({
            right: '',
    ...
    fm.one('init', function () {
      ...
      .one('open', function () {
        opts = fm.api < 2.1 ? null : createsearchmenu();
        ...
        if (opts) {
          ...
          $('#' + id('SearchFromAll'))
            .next('label')
            .attr('title', fm.i18n('searchTarget', fm.i18n('btnAll')));
          if (sTypes) {
            $.each(sTypes, function (i, v) {
              if (v.title) {
                $('#' + id(i))
                  .next('label')
                  .attr('title', fm.i18n(v.title));
              }
            });
          }
          opts
            .on(
              'mousedown',
              'div.buttonset, div.search-row-meat',
              function (e) {
                e.stopPropagation();
      ...
      .bind('open parents', function () {
        ...
        if (volroot) {
          ...
          $('#' + id('SearchFromCwd'))
            ...
            .attr(
              'title',
              fm.i18n('searchTarget', dirs.join(fm.option('separator'))),
            );
          $('#' + id('SearchFromVol'))
            .next('label')
            .attr('title', fm.i18n('searchTarget', volroot.name));
        }
      })
      .bind('open', function () {
        incVal && abort();
      ...
      .shortcut({
        pattern:
          'a b c d e f g h i j k l m n o p q r s t u v w x y z dig0 dig1 dig2 dig3 dig4 dig5 dig6 dig7 dig8 dig9 num0 num1 num2 num3 num4 num5 num6 num7 num8 num9',
          ...
          var code = e.originalEvent.keyCode,
            next = function () {
              var sel = fm.selected(),
                key =
                  $.ui.keyCode[
                    !sel.length || fm.cwdHash2Elm(sel[0]).next('[id]').length
                      ? 'RIGHT'
                      : 'HOME'
                  ];
              $(document).trigger(
