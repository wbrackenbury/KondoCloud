...
$.fn.elfindertoolbar = function (fm, opts) {
  ...
  this.not(".elfinder-toolbar").each(function () {
    var commands = fm._commands,
      ...
      render = function (disabled) {
        ...
        while (l--) {
          if (panels[l]) {
            panel = $(
              `<div class="ui-widget-content ui-corner-all elfinder-buttonset elfinder-buttonset-${panels[
                l
              ].join("-")}"/>`
            );
        ...
        if ((cmdPref = commands["preference"])) {
          ...
          if (
            options.showPreferenceButton === "always" ||
            (!self.children().length && options.showPreferenceButton === "auto")
          ) {
            //cmdPref.state = 0;
            panel = $(
        ...
        !self.data("swipeClose") && self.children().length
          ? self.show()
          : self.hide();
    ...
    self.css("height", "").find(".elfinder-button-text")["show"]();
    fm.trigger("uiresize").storage("toolbarTextLabel", "1");
    ...
    if (options.displayTextLabel !== "none") {
      ...
      if (textLabel === null) {
        textLabel =
          options.displayTextLabel &&
          (!options.labelExcludeUA ||
            !options.labelExcludeUA.length ||
            !$.grep(options.labelExcludeUA, function (v) {
              return fm.UA[v] ? true : false;
            }).length);
      } else {
      ...
      contextRaw.push({
        ...
        callback: function () {
          ...
          self
            .css("height", "")
            .find(".elfinder-button-text")
            [textLabel ? "show" : "hide"]();
          fm.trigger("uiresize").storage(
            "toolbarTextLabel",
            textLabel ? "1" : "0"
          );
        },
        ...
        .on("touchmove touchend", function (e) {
          if (self.data("tmlongtap")) {
            if (
              e.type === "touchend" ||
              Math.abs(
                self.data("longtap").x - e.originalEvent.touches[0].pageX
              ) +
                Math.abs(
                  self.data("longtap").y - e.originalEvent.touches[0].pageY
                ) >
                4
            )
    ...
    if (fm.UA.Touch) {
      ...
      if (typeof autoHide.toolbar === "undefined") {
        autoHide.toolbar =
          options.autoHideUA &&
          options.autoHideUA.length > 0 &&
          $.grep(options.autoHideUA, function (v) {
            return fm.UA[v] ? true : false;
          }).length;
      ...
      if (autoHide.toolbar) {
        fm.one("init", function () {
          fm.uiAutoHide.push(function () {
            self
              .stop(true, true)
              .trigger("toggle", { duration: 500, init: true });
          });
        });
      }
      ...
      self
        .on("toggle", function (e, data) {
          ...
          self
            .data("swipeClose", !toshow)
            .stop(true, true)
            .animate({ height: "toggle" }, opt);
          autoHide.toolbar = !toshow;
          fm.storage(
            "autoHide",
            Object.assign(fm.storage("autoHide"), { toolbar: autoHide.toolbar })
          );
        })
        .on("touchstart", function (e) {
