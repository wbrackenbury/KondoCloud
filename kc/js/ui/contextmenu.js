  ...
  return this.each(function () {
    var self = $(this),
      ...
      dragOpt = {
        distance: 8,
        start: function () {
          menu.data('drag', true).data('touching') &&
            menu.find('.' + cHover).removeClass(cHover);
        },
        stop: function () {
      ...
      menu = $(this)
        .addClass(
          'touch-punch ui-helper-reset ui-front ui-widget ui-state-default ui-corner-all elfinder-contextmenu elfinder-contextmenu-' +
            fm.direction,
        )
        .hide()
        ...
        .on('mouseenter mouseleave', '.' + cmItem, function (e) {
          $(this).toggleClass(
            cHover,
            e.type === 'mouseenter' ||
              (!menu.data('draged') && menu.data('submenuKeep'))
              ? true
              : false,
          );
          if (menu.data('draged') && menu.data('submenuKeep')) {
            menu
              .find('.elfinder-contextmenu-sub:visible')
              .parent()
              .addClass(cHover);
          }
        })
        .on('mouseenter mouseleave', '.' + exIcon, function (e) {
          $(this)
            .parent()
            .toggleClass(cHover, e.type === 'mouseleave');
        })
        .on(
            ...
            if (e.originalEvent) {
              var target = $(this),
                unHover = function () {
                  if (
                    selected &&
                    !selected.children('div.elfinder-contextmenu-sub:visible')
                      .length
                  ) {
      ...
      tpl =
        '<div class="' +
        cmItem +
        '{className}"><span class="elfinder-button-icon {icon} elfinder-contextmenu-icon"{style}/><span>{label}</span></div>',
        ...
        if (opts) {
          ...
          if (opts.iconImg) {
            ...
            style =
              ...
              (pos ? pos : '0 0') +
              ' no-repeat;' +
              (pos ? '' : 'posbackground-size:contain;') +
              '"';
          }
        }
        return $(
            ...
            .data(
              ...
              setTimeout(function () {
                if (menu.is(':visible')) {
                  ...
                  cwd
                    .find('.elfinder-cwd-file.ui-selected')
                    .one(evTouchStart, function (e) {
                      ...
                      if (
                        menu.first().length &&
                        !tgt.is('input:checkbox') &&
                        !tgt.hasClass('elfinder-cwd-select')
                      ) {
                        e.stopPropagation();
                        //e.preventDefault();
                        open(
        ...
        if ($.inArray(code, [ESC, ENT, LEFT, RIGHT, UP, DOWN]) !== -1) {
          ...
          if (code == ESC || code === sublev) {
            ...
            if (subnodes) {
              ...
              if (
                code == DOWN &&
                (!subselected || subnodes.length <= ++subnodes._cur)
              ) {
                subnodes._cur = 0;
      ...
      open = function (x, y, css) {
        var width = menu.outerWidth(),
          ...
          css = Object.assign(css || {}, {
            top: Math.max(
              0,
              y + mh + height < bheight ? y + mh : y - (y + height - bheight),
            ),
            left: Math.max(
              0,
              x < width + mw || x + mw + width < bwidth
                ? x + mw
                : x - mw - width,
            ),
            opacity: '1',
        ...
        if (fm.UA.iOS) {
          $('div.elfinder div.overflow-scrolling-touch').css(
      ...
      create = function (type, targets) {
        ...
        if (selcnt > 1) {
          menu.append(
            '<div class="ui-corner-top ui-widget-header elfinder-contextmenu-header"><span>' +
        ...
        $.each(types[type] || [], function (i, name) {
          ...
          if (
            cmd &&
            !isCwd &&
            (!fm.searchStatus.state || !cmd.disableOnSearch)
          ) {
            cmd.__disabled = cmd._disabled;
            cmd._disabled = !(
              cmd.alwaysEnabled ||
              (fm._commands[cmdName]
                ? $.inArray(name, disabled) === -1 &&
                  (!useMap || !disabled[cmdName])
                : false)
            );
            $.each(cmd.linkedCmds, function (i, n) {
              ...
              if ((c = fm.getCommand(n))) {
                ...
                c._disabled = !(
                  c.alwaysEnabled || (fm._commands[n] ? !disabled[n] : false)
                );
              }
            });
          }
          ...
          if (cmd && !cmd._disabled && cmd.getstate(targets) != -1) {
            if (cmd.variants) {
              ...
              node = item(
                cmd.title,
                cmd.className ? cmd.className : cmd.name,
                function () {},
                cmd.contextmenuOpts,
              );

              submenu = $(
                '<div class="ui-front ui-corner-all elfinder-contextmenu-sub"/>',
              )
                .hide()
                .css('max-height', fm.getUI().height() - 30)
                .appendTo(
                  node.append('<span class="elfinder-contextmenu-arrow"/>'),
                );
              ...
              node
                ...
                .on('mouseenter mouseleave', function (e) {
                  if (!menu.data('touching')) {
                    ...
                    if (
                      !$(e.target).closest('.elfinder-contextmenu-sub', menu)
                        .length
                    ) {
                      if (e.type === 'mouseleave') {
                        ...
                        node.data(
                          ...
                          setTimeout(
                            ...
                            nodes.find('div.elfinder-contextmenu-sub:visible')
                              .length
                              ? 250
                              : 0,
                          ),
              ...
              $.each(cmd.variants, function (i, variant) {
                ...
                if (typeof variant[2] !== 'undefined') {
                  icon = $('<span/>').addClass(
                    'elfinder-button-icon elfinder-contextmenu-icon',
                  );
                  if (!/\//.test(variant[2])) {
            ...
            if (cmd.extendsCmd) {
              node
                .children('span.elfinder-button-icon')
                .addClass('elfinder-button-icon-' + cmd.extendsCmd);
            }
    ...
    fm.one('load', function () {
      ...
      fm.bind('contextmenu', function (e) {
        ...
        if (menu.children().length) {
          ...
          if (data.fitHeight) {
            css = {
              maxHeight: Math.min(fm.getUI().height(), $(window).height()),
              overflowY: 'auto',
            };
        ...
        .bind('select', function (e) {
          currentType === 'files' &&
            (!e.data ||
              e.data.selected.toString() !== currentTargets.toString()) &&
            close();
        });
    }).shortcut({
