...
elFinder.prototype.history = function (fm) {
  ...
  var self = this,
     ...
     */
    nativeHistory =
      fm.options.useBrowserHistory && window.history && window.history.pushState
        ? window.history
        : null,
    /**
    ...
    go = function (fwd) {
      if ((fwd && self.canForward()) || (!fwd && self.canBack())) {
        update = false;
        return fm
          .exec("open", history[fwd ? ++current : --current])
          .fail(reset);
      }
    ...
    setNativeHistory = function (thash) {
      if (
        nativeHistory &&
        (!nativeHistory.state || nativeHistory.state.thash !== thash)
      ) {
        nativeHistory.pushState(
          { thash: thash },
          null,
          location.pathname + location.search + (thash ? "#elf_" + thash : "")
        );
      }
    };
